class DualTextWriter {
     ...
     * 스크립트 섹션 타입 상수
     * 
     * 각 섹션 타입은 고유한 id와 표시 이름을 가집니다.
     * 새로운 섹션 타입을 추가할 때 이 배열을 수정하세요.
     */
    static SECTION_TYPES = ['introduction', 'body', 'conclusion', 'custom'];

    /**
     * 섹션 타입별 표시 이름 매핑
     */
    static SECTION_TYPE_LABELS = {
        'introduction': '서론',
        'body': '본론',
        'conclusion': '결론',
        'custom': '커스텀'
     ...
     * 최대 섹션 수 제한
     */
    static MAX_SECTIONS = 10;

    /**
     * 최소 섹션 수 제한
     */
    static MIN_SECTIONS = 1;

    /**
     * 섹션별 최대 글자 수 제한
     */
    static SECTION_MAX_CHAR_COUNT = 500;

    /**
     * Firebase 컬렉션 이름 상수
     * 
     * 스크립트 데이터를 저장하는 Firestore 컬렉션 이름입니다.
     * 기존 Firestore 스키마와 일치하도록 설정되었습니다.
     */
    static SCRIPT_COLLECTION = 'scripts';

    /**
     * 스크립트 데이터 스키마 정의
     * 
     * @typedef {Object} ScriptSchema
     * @property {string} userId - 사용자 ID (사용자별 데이터 격리)
     * @property {string} title - 스크립트 제목
     * @property {Array<Object>} sections - 섹션 배열
     * @property {string} sections[].id - 섹션 ID
     * @property {string} sections[].type - 섹션 타입 ('introduction' | 'body' | 'conclusion' | 'custom')
     * @property {number} sections[].order - 섹션 순서
     * @property {string} sections[].content - 섹션 내용
     * @property {number} sections[].charCount - 섹션 글자 수
     * @property {boolean} sections[].isComplete - 섹션 완료 여부
     * @property {number} totalCharCount - 전체 글자 수
     * @property {number} completionRate - 완료율 (0-100)
     * @property {firebase.firestore.Timestamp} createdAt - 생성 시간
     * @property {firebase.firestore.Timestamp} updatedAt - 수정 시간
     */
    ...
    constructor() {
        ...
        // 스크립트 작성 탭 관련 요소들
        this.scriptTab = document.querySelector('[data-tab="script"]');
        this.scriptTabContent = document.getElementById('script-tab');
        this.scriptSectionsContainer = document.getElementById('script-sections-container');
        ...
        this.scriptPreviewBtn = document.getElementById('script-preview-btn');
        this.scriptPreviewSection = document.getElementById('script-preview-section');
        this.scriptPreviewContent = document.getElementById('script-preview-content');
        this.scriptSections = []; // 스크립트 섹션 배열 [{id, type, order, content, charCount, isComplete}]
        this.currentScriptId = null; // 현재 편집 중인 스크립트 ID
        this._scriptEventBound = false; // 이벤트 리스너 중복 방지 플래그
        this._isReordering = false; // 순서 변경 중 플래그 (동시 클릭 방지)
        this._resizeTimer = null; // 리사이즈 이벤트 디바운싱 타이머
        this._lastWindowWidth = window.innerWidth; // 마지막 창 너비 (레이아웃 재조정 감지용)
     ...
     * @throws {Error} DOM 요소가 없을 경우 에러 로깅
     */
    initScriptWritingTab() {
        try {
            // 유효성 검사: 필수 DOM 요소 확인
            if (!this.scriptSectionsContainer) {
                console.warn('⚠️ 스크립트 섹션 컨테이너를 찾을 수 없습니다.');
                return;
            }

            // 섹션이 없으면 기본 섹션 생성 (서론/본론/결론)
            if (!this.scriptSections || this.scriptSections.length === 0) {
                this.createDefaultSections();
            }

            // 섹션 렌더링
            this.renderScriptSections();

            // 완료율 업데이트
            this.updateCompletionRate();

            // 이벤트 리스너 바인딩 (한 번만)
            if (!this._scriptEventBound) {
                this._scriptEventBound = true;
                this.bindScriptEvents();
            }
        } catch (error) {
            console.error('❌ 스크립트 작성 탭 초기화 실패:', error);
            if (this.showMessage) {
                this.showMessage('스크립트 작성 탭을 초기화하는 중 오류가 발생했습니다.', 'error');
            }
        }
    }
     ...
     * 기본 섹션 생성 (서론/본론/결론)
     * 
     * @throws {Error} 섹션 생성 실패 시 에러 로깅
     */
    createDefaultSections() {
        try {
            this.scriptSections = [
                this.createScriptSection('introduction', 1),
                this.createScriptSection('body', 2),
                this.createScriptSection('conclusion', 3)
            ];
        } catch (error) {
            console.error('❌ 기본 섹션 생성 실패:', error);
            throw error;
        }
    }
     ...
     * 스크립트 섹션 생성
     * 
     * @param {string} type - 섹션 타입 ('introduction', 'body', 'conclusion', 'custom')
     * @param {number} order - 섹션 순서 (1부터 시작)
     * @param {string} content - 섹션 내용 (선택사항, 기본값: '')
     * @returns {Object} 생성된 섹션 객체
     * @throws {Error} 유효하지 않은 타입이나 순서일 경우 에러
     */
    createScriptSection(type, order, content = '') {
        try {
            // 섹션 타입 유효성 검증
            if (!DualTextWriter.SECTION_TYPES.includes(type)) {
                const error = new Error(`유효하지 않은 섹션 타입입니다: ${type}. 허용된 타입: ${DualTextWriter.SECTION_TYPES.join(', ')}`);
            ...
            if (!Number.isInteger(order) || order < 1) {
                const error = new Error(`유효하지 않은 섹션 순서입니다: ${order}. 순서는 1 이상의 정수여야 합니다.`);
                console.error('❌', error.message);
                throw error;
            }
            ...
            if (typeof content !== 'string') {
                const error = new Error(`섹션 내용은 문자열이어야 합니다.`);
                console.error('❌', error.message);
                throw error;
            }
            ...
            console.error('❌ 스크립트 섹션 생성 실패:', error);
            throw error;
        }
    }
    ...
    renderScriptSections() {
        try {
            ...
            // 섹션을 order 순으로 정렬
            const sortedSections = [...this.scriptSections].sort((a, b) => a.order - b.order);

            // 섹션 HTML 생성
            const sectionsHtml = sortedSections.map((section, index) => {
                return this.renderScriptSection(section, index);
            }).join('');

            // DOM 업데이트 (성능: 한 번의 innerHTML 할당)
            this.scriptSectionsContainer.innerHTML = sectionsHtml;

            // 완료율 업데이트
            this.updateCompletionRate();
        } catch (error) {
            console.error('❌ 스크립트 섹션 렌더링 실패:', error);
            if (this.showMessage) {
                this.showMessage('섹션을 불러오는 중 오류가 발생했습니다.', 'error');
            }
        }
    }
     ...
     * 개별 섹션 렌더링
     * 
     * - 기존 패널 구조와 유사한 패턴 사용 (.writing-panel 스타일 참고)
     * - 일관된 패딩/마진 사용
     * - 접근성 속성 포함 (role, aria-label, aria-level)
     * 
     * @param {Object} section - 섹션 데이터 객체
     * @param {number} index - 섹션 인덱스 (0부터 시작)
     * @returns {string} 섹션 HTML 문자열
     */
    renderScriptSection(section, index) {
        try {
            // 섹션 타입 유효성 검증
            if (!section || !section.type) {
                console.warn('⚠️ 유효하지 않은 섹션 데이터:', section);
                return '';
            }

            // 섹션 타입 라벨 가져오기
            const sectionTypeLabel = DualTextWriter.SECTION_TYPE_LABELS[section.type] || '섹션';
            
            // 섹션 순서 확인
            const sortedSections = [...this.scriptSections].sort((a, b) => a.order - b.order);
            const sectionIndex = sortedSections.findIndex(s => s.id === section.id);
            const isFirst = sectionIndex === 0;
            const isLast = sectionIndex === sortedSections.length - 1;
            
            // XSS 방지: HTML 이스케이프
            const sectionId = this.escapeHtml(section.id);
            const sectionContent = this.escapeHtml(section.content || '');
            const charCount = section.charCount || 0;
            const displayOrder = sectionIndex + 1; // 사용자에게 보여줄 순서 (1부터 시작)

            // 접근성: aria-level은 제목의 중요도를 나타냄 (h2, h3 등)
            const ariaLevel = 2; // 섹션 제목은 h2 수준

            return `
                <div class="script-section" 
                     data-section-id="${sectionId}" 
                     role="region" 
                     aria-label="${sectionTypeLabel} 섹션"
                     aria-labelledby="section-title-${sectionId}">
                    <div class="script-section-header">
                        <div class="script-section-title" id="section-title-${sectionId}">
                            <span class="script-section-number" aria-hidden="true">${displayOrder}</span>
                            <span class="script-section-type" aria-level="${ariaLevel}">${sectionTypeLabel}</span>
                        </div>
                        <div class="script-section-controls" role="group" aria-label="섹션 제어 버튼">
                            <button 
                                class="btn btn-icon script-section-move-up" 
                                data-section-id="${sectionId}"
                                aria-label="섹션 위로 이동"
                                ${isFirst ? 'disabled aria-disabled="true"' : ''}
                                title="${isFirst ? '첫 번째 섹션입니다' : '위로 이동'}"
                                tabindex="${isFirst ? '-1' : '0'}"
                            >
                                <span aria-hidden="true">▲</span>
                            </button>
                            <button 
                                class="btn btn-icon script-section-move-down" 
                                data-section-id="${sectionId}"
                                aria-label="섹션 아래로 이동"
                                ${isLast ? 'disabled aria-disabled="true"' : ''}
                                title="${isLast ? '마지막 섹션입니다' : '아래로 이동'}"
                                tabindex="${isLast ? '-1' : '0'}"
                            >
                                <span aria-hidden="true">▼</span>
                            </button>
                            <button 
                                class="script-section-delete btn-icon" 
                                data-section-id="${sectionId}"
                                aria-label="섹션 삭제"
                                ${this.scriptSections.length <= DualTextWriter.MIN_SECTIONS ? 'disabled aria-disabled="true"' : ''}
                                title="${this.scriptSections.length <= DualTextWriter.MIN_SECTIONS ? '최소 1개 섹션이 필요합니다' : '섹션 삭제'}"
                            >
                                🗑️
                            </button>
                        </div>
                    ...
                    <div class="script-section-body">
                        <textarea 
                            class="script-section-content" 
                            data-section-id="${sectionId}"
                            placeholder="${sectionTypeLabel} 내용을 입력하세요..."
                            maxlength="${DualTextWriter.SECTION_MAX_CHAR_COUNT}"
                            aria-label="${sectionTypeLabel} 내용 입력"
                            aria-required="false"
                            aria-describedby="char-count-${sectionId}"
                        >${sectionContent}</textarea>
                        <div class="script-section-footer">
                            <div class="character-counter script-section-char-count" id="char-count-${sectionId}" aria-live="polite" aria-atomic="true" role="status" aria-label="글자 수">
                                <span class="char-count-current" data-section-id="${sectionId}" aria-label="현재 글자 수">${charCount}</span>
                                <span class="separator" aria-hidden="true">/</span>
                                <span class="char-count-max" aria-label="최대 글자 수">${DualTextWriter.SECTION_MAX_CHAR_COUNT}</span>
                                <span class="unit" aria-hidden="true">자</span>
                                <span class="sr-only">현재 ${charCount}자, 최대 ${DualTextWriter.SECTION_MAX_CHAR_COUNT}자</span>
                            </div>
                            <label class="script-section-complete">
                                <input 
                                    type="checkbox" 
                                    class="script-section-checkbox" 
                                    data-section-id="${sectionId}"
                                    ${section.isComplete ? 'checked' : ''}
                                    role="checkbox"
                                    aria-checked="${section.isComplete ? 'true' : 'false'}"
                                    aria-label="${sectionTypeLabel} 완료 체크"
                                />
                                <span>완료</span>
                            </label>
                        </div>
     ...
     * - 미리보기 버튼
     * 
     * @throws {Error} 이벤트 바인딩 실패 시 에러 로깅
     */
    bindScriptEvents() {
        try {
            // 섹션 추가 버튼
            ...
            // 미리보기 버튼
            if (this.scriptPreviewBtn) {
                this.scriptPreviewBtn.addEventListener('click', () => {
                    this.togglePreview();
                });
            }

            // 이벤트 위임: 섹션 컨테이너 내부 이벤트 처리
            if (this.scriptSectionsContainer) {
                // 섹션 순서 변경 (위로) - 클릭 및 키보드 이벤트
                this.scriptSectionsContainer.addEventListener('click', (e) => {
                    const moveUpBtn = e.target.closest('.script-section-move-up');
                    if (moveUpBtn && !moveUpBtn.disabled) {
                        const sectionId = moveUpBtn.getAttribute('data-section-id');
                        if (sectionId) {
                ...
                // 키보드 접근 (Enter/Space)
                this.scriptSectionsContainer.addEventListener('keydown', (e) => {
                    const moveUpBtn = e.target.closest('.script-section-move-up');
                    if (moveUpBtn && !moveUpBtn.disabled && (e.key === 'Enter' || e.key === ' ')) {
                        e.preventDefault();
                        const sectionId = moveUpBtn.getAttribute('data-section-id');
                        if (sectionId) {
                            this.moveSectionUp(sectionId);
                ...
                // 섹션 순서 변경 (아래로) - 클릭 및 키보드 이벤트
                this.scriptSectionsContainer.addEventListener('click', (e) => {
                    const moveDownBtn = e.target.closest('.script-section-move-down');
                    if (moveDownBtn && !moveDownBtn.disabled) {
                        const sectionId = moveDownBtn.getAttribute('data-section-id');
                        if (sectionId) {
                ...
                this.scriptSectionsContainer.addEventListener('keydown', (e) => {
                    const moveDownBtn = e.target.closest('.script-section-move-down');
                    if (moveDownBtn && !moveDownBtn.disabled && (e.key === 'Enter' || e.key === ' ')) {
                        e.preventDefault();
                        const sectionId = moveDownBtn.getAttribute('data-section-id');
                        if (sectionId) {
                            this.moveSectionDown(sectionId);
                ...
                // 섹션 삭제
                this.scriptSectionsContainer.addEventListener('click', (e) => {
                    const deleteBtn = e.target.closest('.script-section-delete');
                    if (deleteBtn && !deleteBtn.disabled) {
                        const sectionId = deleteBtn.getAttribute('data-section-id');
                        if (sectionId) {
                ...
                // 섹션 내용 입력 (실시간 업데이트)
                this.scriptSectionsContainer.addEventListener('input', (e) => {
                    const textarea = e.target.closest('.script-section-content');
                    if (textarea) {
                        const sectionId = textarea.getAttribute('data-section-id');
                        if (sectionId) {
                            // maxlength 체크 (추가 검증)
                            const currentValue = textarea.value;
                            if (currentValue.length > DualTextWriter.SECTION_MAX_CHAR_COUNT) {
                                // maxlength 속성으로 이미 제한되지만, 추가 안전장치
                                textarea.value = currentValue.substring(0, DualTextWriter.SECTION_MAX_CHAR_COUNT);
                            }
                            this.updateSectionContent(sectionId, textarea.value);
                        }
                    }
                });

                // maxlength 초과 시 경고 (keydown 이벤트로 사전 차단)
                this.scriptSectionsContainer.addEventListener('keydown', (e) => {
                    const textarea = e.target.closest('.script-section-content');
                    if (textarea && e.key !== 'Backspace' && e.key !== 'Delete' && e.key !== 'ArrowLeft' && e.key !== 'ArrowRight') {
                        const currentValue = textarea.value;
                        const selectionStart = textarea.selectionStart;
                        const selectionEnd = textarea.selectionEnd;
                        const selectedText = currentValue.substring(selectionStart, selectionEnd);
                        const newLength = currentValue.length - selectedText.length + 1; // 입력될 문자 1개 추가
                        
                        if (newLength > DualTextWriter.SECTION_MAX_CHAR_COUNT) {
                            e.preventDefault();
                            if (!this._charLimitWarningShown) {
                                this._charLimitWarningShown = true;
                                if (this.showMessage) {
                                    this.showMessage(`섹션당 최대 ${DualTextWriter.SECTION_MAX_CHAR_COUNT}자까지 입력할 수 있습니다.`, 'warning');
                                }
                                setTimeout(() => {
                                    this._charLimitWarningShown = false;
                                }, 3000);
                            }
                        }
                    }
                ...
                // 섹션 완료 체크박스
                this.scriptSectionsContainer.addEventListener('change', (e) => {
                    const checkbox = e.target.closest('.script-section-checkbox');
                    if (checkbox) {
                        const sectionId = checkbox.getAttribute('data-section-id');
                        if (sectionId) {
                            this.toggleSectionComplete(sectionId, checkbox.checked);
                        }
                    }
                });
            }
        } catch (error) {
            console.error('❌ 스크립트 이벤트 바인딩 실패:', error);
            if (this.showMessage) {
                this.showMessage('스크립트 기능을 초기화하는 중 오류가 발생했습니다.', 'error');
            }
        }
    }
     ...
     * - 섹션 추가 시 포커스 관리
     * 
     * @param {string} [type='custom'] - 섹션 타입 ('introduction', 'body', 'conclusion', 'custom')
     * @throws {Error} 섹션 추가 실패 시 에러 로깅
     */
    addNewSection(type = 'custom') {
        try {
            // 섹션 타입 유효성 검증
            if (!DualTextWriter.SECTION_TYPES.includes(type)) {
                console.warn('⚠️ 유효하지 않은 섹션 타입:', type);
                type = 'custom';
            }

            // 최대 섹션 수 제한 체크
            if (this.scriptSections.length >= DualTextWriter.MAX_SECTIONS) {
                const errorMessage = `최대 ${DualTextWriter.MAX_SECTIONS}개까지 섹션을 추가할 수 있습니다.`;
            ...
            // 현재 최대 order 값 찾기
            const maxOrder = this.scriptSections.length > 0 
                ? Math.max(...this.scriptSections.map(s => s.order))
                : 0;

            // 새 섹션 생성 (빈 섹션 허용)
            const newSection = this.createScriptSection(type, maxOrder + 1, '');

            // 섹션 배열에 추가
            this.scriptSections.push(newSection);
            
            // UI 업데이트
            this.renderScriptSections();

            // 새로 추가된 섹션에 애니메이션 적용 및 포커스 이동
            setTimeout(() => {
                const newSectionElement = document.querySelector(`[data-section-id="${newSection.id}"].script-section`);
                const newTextarea = document.querySelector(`[data-section-id="${newSection.id}"].script-section-content`);
                
                if (newSectionElement) {
                    // 부드러운 애니메이션: 페이드인 + 슬라이드인
                    newSectionElement.style.opacity = '0';
                    newSectionElement.style.transform = 'translateY(-10px)';
                    newSectionElement.style.transition = 'opacity 0.3s ease, transform 0.3s ease';
            ...
            if (this.showMessage) {
                this.showMessage('섹션을 추가하는 중 오류가 발생했습니다.', 'error');
            }
        }
    }
     ...
     * @throws {Error} 이동 실패 시 에러 로깅
     */
    moveSectionUp(sectionId) {
        try {
            // 동시 클릭 방지 (debounce)
            if (this._isReordering) {
                console.warn('⚠️ 이미 순서 변경 중입니다. 잠시 후 다시 시도해주세요.');
                return;
            }

            // 섹션 찾기
            const section = this.scriptSections.find(s => s.id === sectionId);
            if (!section) {
            ...
            // 이전 섹션 찾기
            const prevSection = this.scriptSections.find(s => s.order === section.order - 1);
            if (!prevSection) {
                console.warn('⚠️ 이전 섹션을 찾을 수 없습니다.');
                return;
            }
            ...
            try {
                ...
                prevSection.updatedAt = new Date();

                // 순서 재조정 (reorderSections 호출)
                this.reorderSections();

                // UI 업데이트 (부분 리렌더링)
                this.updateSectionOrderUI(sectionId, prevSection.id);

                // 섹션 순서 저장 (Firebase)
                await this.saveSectionOrder();
            } catch (reorderError) {
                // 롤백: 원래 순서로 복구
                section.order = originalOrder.section;
                prevSection.order = originalOrder.prevSection;
                console.error('❌ 순서 변경 실패, 롤백:', reorderError);
                if (this.showMessage) {
                    this.showMessage('섹션 순서를 변경하는 중 오류가 발생했습니다.', 'error');
                }
            } finally {
                // debounce 해제 (300ms 후)
                setTimeout(() => {
            ...
            console.error('❌ 섹션 위로 이동 실패:', error);
            if (this.showMessage) {
                this.showMessage('섹션 순서를 변경하는 중 오류가 발생했습니다.', 'error');
            }
            this._isReordering = false;
    ...
    moveSectionDown(sectionId) {
        try {
            ...
            // 마지막 섹션 체크
            const maxOrder = Math.max(...this.scriptSections.map(s => s.order));
            if (section.order >= maxOrder) {
                return; // 이미 마지막
            }

            // 다음 섹션 찾기
            const nextSection = this.scriptSections.find(s => s.order === section.order + 1);
            if (!nextSection) {
                console.warn('⚠️ 다음 섹션을 찾을 수 없습니다.');
                return;
            }
            ...
            try {
                ...
                section.updatedAt = new Date();
                nextSection.updatedAt = new Date();

                // 순서 재조정 (reorderSections 호출)
                this.reorderSections();

                // UI 업데이트 (부분 리렌더링)
                this.updateSectionOrderUI(sectionId, nextSection.id);
                ...
                // 롤백: 원래 순서로 복구
                section.order = originalOrder.section;
                nextSection.order = originalOrder.nextSection;
                console.error('❌ 순서 변경 실패, 롤백:', reorderError);
                if (this.showMessage) {
                    this.showMessage('섹션 순서를 변경하는 중 오류가 발생했습니다.', 'error');
                }
            ...
            console.error('❌ 섹션 아래로 이동 실패:', error);
            if (this.showMessage) {
                this.showMessage('섹션 순서를 변경하는 중 오류가 발생했습니다.', 'error');
            }
            this._isReordering = false;
     ...
     * 섹션 순서 재조정
     * 
     * - 모든 섹션의 order를 1부터 연속적으로 재조정
     * - 순서 변경 후 호출하여 일관성 유지
     * 
     * @throws {Error} 재조정 실패 시 에러 로깅
     */
    reorderSections() {
        try {
            // 섹션을 order 순으로 정렬
            ...
            console.error('❌ 섹션 순서 재조정 실패:', error);
            throw error; // 상위 함수에서 롤백 처리
        }
    }

    /**
     * 섹션 순서 UI 업데이트 (부분 리렌더링)
     * 
     * - 필요한 부분만 DOM 업데이트
     * - CSS transform으로 애니메이션 최적화
     * 
     * @param {string} sectionId1 - 첫 번째 섹션 ID
     * @param {string} sectionId2 - 두 번째 섹션 ID
     */
    updateSectionOrderUI(sectionId1, sectionId2) {
        try {
            const section1 = document.querySelector(`[data-section-id="${sectionId1}"].script-section`);
            const section2 = document.querySelector(`[data-section-id="${sectionId2}"].script-section`);

            if (!section1 || !section2) {
                // DOM 요소를 찾을 수 없으면 전체 리렌더링
                this.renderScriptSections();
                return;
            }

            // CSS transform으로 애니메이션 (성능 최적화)
            section1.style.transition = 'transform 0.3s ease';
            section2.style.transition = 'transform 0.3s ease';

            // 임시로 위치 교환 애니메이션
            const temp = section1.cloneNode(true);
            const parent = section1.parentNode;
            const nextSibling = section1.nextSibling;
            ...
            // 새로운 순서로 삽입
            if (nextSibling) {
                parent.insertBefore(section2, nextSibling);
            ...
            const sortedSections = [...this.scriptSections].sort((a, b) => a.order - b.order);
            sortedSections.forEach((section, index) => {
                const sectionElement = document.querySelector(`[data-section-id="${section.id}"].script-section`);
                if (sectionElement) {
                    const numberElement = sectionElement.querySelector('.script-section-number');
                    if (numberElement) {
                        numberElement.textContent = index + 1;
                    }

                    // 버튼 상태 업데이트
                    const moveUpBtn = sectionElement.querySelector('.script-section-move-up');
                    const moveDownBtn = sectionElement.querySelector('.script-section-move-down');
                    const isFirst = index === 0;
                    const isLast = index === sortedSections.length - 1;

                    if (moveUpBtn) {
                        moveUpBtn.disabled = isFirst;
                        moveUpBtn.setAttribute('aria-disabled', isFirst ? 'true' : 'false');
                        moveUpBtn.setAttribute('tabindex', isFirst ? '-1' : '0');
                    }

                    if (moveDownBtn) {
                        moveDownBtn.disabled = isLast;
                        moveDownBtn.setAttribute('aria-disabled', isLast ? 'true' : 'false');
                        moveDownBtn.setAttribute('tabindex', isLast ? '-1' : '0');
                    }
                }
            });
            ...
            setTimeout(() => {
                section1.style.transition = '';
                section2.style.transition = '';
            }, 300);
        } catch (error) {
            console.error('❌ 섹션 순서 UI 업데이트 실패:', error);
            // 실패 시 전체 리렌더링
            this.renderScriptSections();
        }
    }
     ...
     * - 스크린 리더 지원
     * 
     * @param {string} sectionId - 삭제할 섹션 ID
     * @param {string} sectionType - 섹션 타입 (라벨 표시용)
     * @returns {Promise<boolean>} 삭제 확인 여부
     */
    async confirmSectionDeletion(sectionId, sectionType = '') {
        return new Promise((resolve) => {
            try {
                // 섹션 타입 라벨 가져오기
                const sectionTypeLabel = DualTextWriter.SECTION_TYPE_LABELS[sectionType] || '섹션';
                ...
                modal.setAttribute('aria-labelledby', 'delete-section-modal-title');
                modal.style.display = 'flex';

                modal.innerHTML = `
                    <div class="modal-content" style="max-width: 400px;">
                        <h3 id="delete-section-modal-title" style="margin: 0 0 16px 0; font-size: 1.2rem; color: #2c3e50;">
                            ⚠️ 섹션 삭제 확인
                        </h3>
                        <p style="margin: 0 0 24px 0; color: #555; line-height: 1.6;">
                            ${sectionTypeLabel} 섹션을 삭제하시겠습니까?<br>
                            <strong style="color: #e74c3c;">이 작업은 되돌릴 수 없습니다.</strong>
                        </p>
                ...
                // 포커스 관리: 모달이 열릴 때 첫 번째 버튼에 포커스
                const cancelBtn = modal.querySelector('#delete-section-cancel-btn');
                const confirmBtn = modal.querySelector('#delete-section-confirm-btn');
                ...
                console.error('❌ 삭제 확인 다이얼로그 생성 실패:', error);
                // 에러 발생 시 기본 confirm 사용
                resolve(confirm('이 섹션을 삭제하시겠습니까?'));
            }
        });
    }
     ...
     * 
     * @param {string} sectionId - 삭제할 섹션 ID
     * @throws {Error} 섹션 삭제 실패 시 에러 로깅
     */
    async deleteSection(sectionId) {
        try {
            // 섹션 ID 유효성 검증
            if (!sectionId || typeof sectionId !== 'string') {
                console.warn('⚠️ 유효하지 않은 섹션 ID:', sectionId);
                return;
            }

            // 최소 섹션 수 제한 체크
            if (this.scriptSections.length <= DualTextWriter.MIN_SECTIONS) {
                const errorMessage = `최소 ${DualTextWriter.MIN_SECTIONS}개 섹션이 필요합니다.`;
                console.warn('⚠️', errorMessage);
                if (this.showMessage) {
                    this.showMessage(errorMessage, 'warning');
                }
            ...
            // 삭제할 섹션 찾기
            const sectionToDelete = this.scriptSections.find(s => s.id === sectionId);
            if (!sectionToDelete) {
                console.warn('⚠️ 삭제할 섹션을 찾을 수 없습니다:', sectionId);
                return;
            }

            // 삭제 확인 (접근성 있는 모달 다이얼로그)
            const confirmed = await this.confirmSectionDeletion(sectionId, sectionToDelete.type);
            if (!confirmed) {
                return;
            }

            // 이전 상태 저장 (롤백용)
            const previousSections = [...this.scriptSections];
            const sectionIndex = this.scriptSections.findIndex(s => s.id === sectionId);

            // 섹션 제거
            if (sectionIndex >= 0) {
                this.scriptSections.splice(sectionIndex, 1);
                
                // 순서 재조정 최적화 (한 번의 순회로 처리)
                this.reorderSections();
                ...
                // 성공 메시지
                if (this.showMessage) {
                    this.showMessage('섹션이 삭제되었습니다.', 'success');
                }
            }
        } catch (error) {
            console.error('❌ 섹션 삭제 실패:', error);
            if (this.showMessage) {
                this.showMessage('섹션을 삭제하는 중 오류가 발생했습니다.', 'error');
            }
            // 에러 발생 시 롤백 (선택적)
            // this.scriptSections = previousSections;
            // this.renderScriptSections();
        }
    }
     ...
     * @throws {Error} 업데이트 실패 시 에러 로깅
     */
    updateSectionContent(sectionId, content) {
        try {
            // 입력값 검증
            if (typeof content !== 'string') {
                console.warn('⚠️ 섹션 내용은 문자열이어야 합니다.');
                return;
            }
            ...
            if (charCount > DualTextWriter.SECTION_MAX_CHAR_COUNT) {
                // maxlength 속성으로 이미 입력이 제한되지만, 추가 검증
                const trimmedContent = content.substring(0, DualTextWriter.SECTION_MAX_CHAR_COUNT);
            ...
            // 대용량 텍스트 처리 (10,000자 이상 시 성능 최적화)
            if (charCount > 10000) {
                // 대용량 텍스트는 즉시 업데이트하지 않고 디바운싱 시간 증가
                if (this._charCountUpdateTimer) {
                    clearTimeout(this._charCountUpdateTimer);
                }
                this._charCountUpdateTimer = setTimeout(() => {
                    this.updateSectionCharCount(sectionId);
                }, 500); // 대용량일 때는 500ms로 증가
            }

            // 내용 업데이트
            section.content = content;
            section.charCount = content.length;
            section.updatedAt = new Date();

            // 글자 수 업데이트 (디바운싱으로 성능 최적화)
            if (charCount <= 10000) {
                // 일반 텍스트는 300ms 디바운싱
                if (this._charCountUpdateTimer) {
                    clearTimeout(this._charCountUpdateTimer);
            ...
            // textarea의 실제 값도 업데이트 (maxlength로 제한된 경우)
            const textarea = document.querySelector(`[data-section-id="${sectionId}"].script-section-content`);
            if (textarea && textarea.value !== content) {
                textarea.value = content;
            }
        } catch (error) {
            console.error('❌ 섹션 내용 업데이트 실패:', error);
            if (this.showMessage) {
                this.showMessage('섹션 내용을 업데이트하는 중 오류가 발생했습니다.', 'error');
            }
        }
    }
     ...
     * 섹션 글자 수 업데이트
     * 
     * - 기존 글자 수 카운터와 동일한 패턴 사용
     * - 글자 수 제한 초과 시 시각적 피드백
     * - 접근성: aria-live로 실시간 업데이트 알림
     * - 스크린 리더 친화적 형식
     * 
     * @param {string} sectionId - 섹션 ID
     * @throws {Error} 글자 수 계산 실패 시 에러 로깅
     */
    updateSectionCharCount(sectionId) {
        try {
            // 섹션 찾기
            const section = this.scriptSections.find(s => s.id === sectionId);
            if (!section) {
            ...
            let charCount = 0;
            try {
                charCount = section.charCount || (section.content ? section.content.length : 0);
                
                // 유효성 검증
                if (typeof charCount !== 'number' || charCount < 0) {
                    console.warn('⚠️ 유효하지 않은 글자 수:', charCount);
                    charCount = 0;
                }
            } catch (calcError) {
                console.error('❌ 글자 수 계산 실패:', calcError);
                charCount = 0;
            }

            const maxCount = DualTextWriter.SECTION_MAX_CHAR_COUNT;

            // 글자 수 표시 업데이트
            const charCountElement = document.querySelector(`[data-section-id="${sectionId}"].char-count-current`);
            if (charCountElement) {
                charCountElement.textContent = charCount;
                charCountElement.setAttribute('aria-label', `현재 글자 수 ${charCount}자`);
                
                // 글자 수 제한 초과 시 시각적 피드백 (색상 변경)
                if (charCount >= maxCount) {
                    charCountElement.style.color = '#dc3545'; // 빨간색
                    charCountElement.style.fontWeight = '600';
                } else if (charCount >= maxCount * 0.9) {
                    charCountElement.style.color = '#f97316'; // 주황색 (90% 이상)
                    charCountElement.style.fontWeight = '600';
                } else {
                    charCountElement.style.color = '#667eea'; // 기본 색상
                    charCountElement.style.fontWeight = '600';
                }
            }

            // 스크린 리더용 숨겨진 텍스트 업데이트
            const srOnlyElement = document.querySelector(`#char-count-${sectionId} .sr-only`);
            if (srOnlyElement) {
                srOnlyElement.textContent = `현재 ${charCount}자, 최대 ${maxCount}자`;
            }

            // textarea 스타일 업데이트 (제한 초과 시)
            const textarea = document.querySelector(`[data-section-id="${sectionId}"].script-section-content`);
            if (textarea) {
                if (charCount >= maxCount) {
                    textarea.style.borderColor = '#dc3545';
                    textarea.style.boxShadow = '0 0 0 3px rgba(220, 53, 69, 0.1)';
                } else if (charCount >= maxCount * 0.9) {
            ...
            // 섹션 데이터 동기화 (charCount 업데이트)
            section.charCount = charCount;
        } catch (error) {
            console.error('❌ 섹션 글자 수 업데이트 실패:', error);
            if (this.showMessage) {
                this.showMessage('글자 수를 업데이트하는 중 오류가 발생했습니다.', 'error');
            }
        }
    }
     ...
     * - 체크 상태 변경 시 즉시 피드백
     * - 배치 저장 고려
     * - 접근성: aria-checked 상태 관리
     * 
     * @param {string} sectionId - 섹션 ID
     * @param {boolean} isComplete - 완료 여부
     * @throws {Error} 토글 실패 시 에러 로깅
     */
    toggleSectionComplete(sectionId, isComplete) {
        try {
            // 입력값 검증
            if (typeof isComplete !== 'boolean') {
                console.warn('⚠️ 유효하지 않은 완료 상태:', isComplete);
                return;
            }
            ...
            // 이전 상태 저장 (롤백용)
            const previousState = section.isComplete;
            ...
            // 즉시 피드백: UI 업데이트
            const checkbox = document.querySelector(`[data-section-id="${sectionId}"].script-section-checkbox`);
            if (checkbox) {
                checkbox.checked = isComplete;
                checkbox.setAttribute('aria-checked', isComplete ? 'true' : 'false');
            }

            // 완료율 업데이트
            this.updateCompletionRate();

            // 배치 저장 고려: 디바운싱으로 Firebase 저장
            if (this._completeStateSaveTimer) {
                clearTimeout(this._completeStateSaveTimer);
            }
            this._completeStateSaveTimer = setTimeout(async () => {
                try {
                    await this.saveSectionCompleteState(sectionId, isComplete);
                } catch (saveError) {
                    // 저장 실패 시 롤백
                    console.error('❌ 체크 상태 저장 실패, 롤백:', saveError);
                    section.isComplete = previousState;
                    if (checkbox) {
                        checkbox.checked = previousState;
                        checkbox.setAttribute('aria-checked', previousState ? 'true' : 'false');
                    }
                    if (this.showMessage) {
                        this.showMessage('체크 상태를 저장하는 중 오류가 발생했습니다.', 'error');
                    }
                }
            }, 1000); // 1초 디바운싱 (배치 저장)
        } catch (error) {
            console.error('❌ 섹션 완료 상태 토글 실패:', error);
            if (this.showMessage) {
                this.showMessage('완료 상태를 변경하는 중 오류가 발생했습니다.', 'error');
            }
        }
    }

    /**
     * 섹션 완료 상태 저장 (Firebase)
     * 
     * - 체크 상태 저장 실패 시 처리
     * - 네트워크 오류 처리
     * 
     ...
     */
    async saveSectionCompleteState(sectionId, isComplete) {
        try {
            // Firebase 준비 확인
            if (!this.isFirebaseReady || !this.db) {
                console.warn('⚠️ Firebase가 준비되지 않았습니다. 체크 상태는 로컬에만 저장됩니다.');
                return;
            }

            // 사용자 확인
            if (!this.currentUser) {
                console.warn('⚠️ 로그인이 필요합니다. 체크 상태는 로컬에만 저장됩니다.');
                return;
            }

            // 스크립트 ID 확인
            if (!this.currentScriptId) {
                console.warn('⚠️ 저장된 스크립트가 없습니다. 체크 상태는 로컬에만 저장됩니다.');
                return;
            }

            // Firebase 업데이트 (배치 저장 고려: 단일 필드만 업데이트)
            const scriptRef = this.db.collection(DualTextWriter.SCRIPT_COLLECTION).doc(this.currentScriptId);
            ...
            // 섹션 배열에서 해당 섹션만 업데이트
            const currentSections = scriptDoc.data().sections || [];
            ...
            // 완료율 계산
            const completedCount = updatedSections.filter(s => s.isComplete).length;
            ...
            console.log(`✅ 섹션 완료 상태 저장 완료: ${sectionId} = ${isComplete}`);
        } catch (error) {
            console.error('❌ 섹션 완료 상태 저장 실패:', error);
            
            // 네트워크 오류 처리
            if (error.code === 'unavailable' || error.code === 'deadline-exceeded') {
                throw new Error('네트워크 오류로 인해 체크 상태를 저장할 수 없습니다.');
            }
     ...
     * 완료율 계산 및 업데이트
     * 
     * - 기존 진행률 표시와 일치 (.progress-bar 스타일 재사용)
     * - 섹션이 없을 경우 처리 (0% 또는 숨김)
     * - 완료율 계산 최적화
     * - 접근성: role="progressbar", aria-label 추가
     * 
     * @throws {Error} 업데이트 실패 시 에러 로깅
     */
    updateCompletionRate() {
        try {
            // 섹션이 없을 경우 처리
            if (!this.scriptSections || this.scriptSections.length === 0) {
                const completionSection = document.getElementById('script-completion-section');
            ...
            // 완료율 계산 최적화 (한 번의 필터링으로 계산)
            const completedCount = this.scriptSections.filter(s => s.isComplete).length;
            const totalCount = this.scriptSections.length;
            ...
            // 완료율 텍스트 업데이트
            const completionRateElement = document.getElementById('script-completion-rate');
            if (completionRateElement) {
                completionRateElement.textContent = `${completionRate}%`;
            }

            // 진행률 바 업데이트
            const progressFill = document.getElementById('script-completion-fill');
            const progressBar = document.getElementById('script-completion-progressbar');
            if (progressFill && progressBar) {
                progressFill.style.width = `${completionRate}%`;
                progressBar.setAttribute('aria-valuenow', completionRate.toString());
                progressBar.setAttribute('aria-label', `완료율: ${completionRate}%`);
            }

            // 상세 정보 업데이트 (스크린 리더용)
            const completionDetail = document.getElementById('script-completion-detail');
            if (completionDetail) {
                completionDetail.textContent = `완료된 섹션: ${completedCount}개, 전체 섹션: ${totalCount}개`;
            }
        } catch (error) {
    ...
    togglePreview() {
        try {
            if (!this.scriptPreviewSection) {
                console.warn('⚠️ 미리보기 섹션을 찾을 수 없습니다.');
                return;
            }

            const isVisible = this.scriptPreviewSection.style.display !== 'none';
            
            if (isVisible) {
                // 미리보기 숨기기
                this.scriptPreviewSection.style.display = 'none';
                if (this.scriptPreviewBtn) {
                    this.scriptPreviewBtn.textContent = '👁️ 미리보기';
                }
            } else {
                // 미리보기 표시
                this.renderFullPreview();
                this.scriptPreviewSection.style.display = 'block';
                if (this.scriptPreviewBtn) {
                    this.scriptPreviewBtn.textContent = '👁️ 숨기기';
                }

                // 미리보기 섹션으로 스크롤
                this.scriptPreviewSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
            }
        } catch (error) {
            console.error('❌ 미리보기 토글 실패:', error);
            if (this.showMessage) {
                this.showMessage('미리보기를 표시하는 중 오류가 발생했습니다.', 'error');
            }
        }
    }
     ...
     */
    /**
     * 전체 스크립트를 하나의 텍스트로 결합
     * 
     * - 빈 섹션 처리 (빈 섹션은 제목만 포함)
     * - 대용량 텍스트 처리 (성능 최적화)
     * 
     * @returns {string} 결합된 전체 스크립트 텍스트
     */
    getCombinedScript() {
        try {
            // 섹션을 order 순으로 정렬
            ...
            // 전체 글 결합 (빈 섹션 처리 포함)
            const combinedParts = sortedSections.map((section, index) => {
                const sectionTypeLabel = DualTextWriter.SECTION_TYPE_LABELS[section.type] || '섹션';
                const sectionContent = (section.content || '').trim();
                
                // 빈 섹션 처리: 제목만 포함
                if (!sectionContent) {
                    return `${index + 1}. ${sectionTypeLabel}\n(내용 없음)`;
                }
            ...
            console.error('❌ 전체 스크립트 결합 실패:', error);
            return '';
        }
    }
     ...
     * - 기존 미리보기 스타일과 일치
     * - 빈 섹션 처리
     * - 대용량 텍스트 처리 (가상 스크롤 고려)
     * - 접근성: role="region", aria-label
     * 
     * @throws {Error} 렌더링 실패 시 에러 로깅
     */
    renderFullPreview() {
        try {
            if (!this.scriptPreviewContent) {
                console.warn('⚠️ 미리보기 콘텐츠 영역을 찾을 수 없습니다.');
                return;
            }
            ...
            // 섹션이 없을 경우 처리
            if (!this.scriptSections || this.scriptSections.length === 0) {
                this.scriptPreviewContent.innerHTML = '<p class="preview-empty">작성된 내용이 없습니다.</p>';
                return;
            }
            ...
            // 전체 글 HTML 생성 (빈 섹션 처리 포함)
            const previewHtml = sortedSections.map((section, index) => {
                const sectionTypeLabel = DualTextWriter.SECTION_TYPE_LABELS[section.type] || '섹션';
                const sectionContent = (section.content || '').trim();
                const escapedContent = this.escapeHtml(sectionContent);

                // 빈 섹션 처리
                if (!sectionContent) {
                    return `
                        <div class="preview-section" role="region" aria-label="${sectionTypeLabel} 미리보기">
                            <h4 class="preview-section-title">${index + 1}. ${sectionTypeLabel}</h4>
                            <div class="preview-section-content"><em class="preview-empty">내용이 없습니다.</em></div>
                        </div>
                    `;
                }
                ...
                return `
                    <div class="preview-section" role="region" aria-label="${sectionTypeLabel} 미리보기">
                        <h4 class="preview-section-title">${index + 1}. ${sectionTypeLabel}</h4>
                        <div class="preview-section-content">${escapedContent}</div>
                    </div>
                `;
            }).join('');

            // 대용량 텍스트 처리: 가상 스크롤 고려 (현재는 전체 렌더링, 필요시 개선)
            this.scriptPreviewContent.innerHTML = previewHtml;

            // 미리보기 섹션으로 스크롤 (부드러운 스크롤)
            if (this.scriptPreviewSection) {
                setTimeout(() => {
                    this.scriptPreviewSection.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
                }, 100);
            }
        } catch (error) {
            console.error('❌ 전체 미리보기 렌더링 실패:', error);
            if (this.showMessage) {
                this.showMessage('미리보기를 생성하는 중 오류가 발생했습니다.', 'error');
            }
            // 에러 발생 시 빈 상태 표시
            if (this.scriptPreviewContent) {
                this.scriptPreviewContent.innerHTML = '<p class="preview-empty">미리보기를 생성하는 중 오류가 발생했습니다.</p>';
            }
        }
    }
     ...
     * - 저장 중 로딩 상태 알림
     * - 저장 완료 메시지
     * - Firebase 트랜잭션 사용 (배치 저장으로 네트워크 요청 최소화)
     * 
     * @throws {Error} 저장 실패 시 에러 로깅
     */
    async saveScript() {
        try {
            // Firebase 준비 확인
            if (!this.isFirebaseReady || !this.db) {
                if (this.showMessage) {
                    this.showMessage('Firebase가 준비되지 않았습니다. 잠시 후 다시 시도해주세요.', 'warning');
                }
                return;
            }
            ...
            if (!this.scriptSections || this.scriptSections.length === 0) {
                if (this.showMessage) {
                    this.showMessage('저장할 섹션이 없습니다. 섹션을 추가해주세요.', 'warning');
                }
                return;
            }

            // 제목 가져오기 (입력값 검증)
            const title = this.scriptTitleInput ? this.scriptTitleInput.value.trim() : '';
            if (title.length > 100) {
                if (this.showMessage) {
                    this.showMessage('제목은 100자 이하여야 합니다.', 'warning');
                }
                return;
            }

            // 저장 중 로딩 상태 알림
            const saveBtn = this.scriptSaveBtn;
            let originalBtnText = '';
            if (saveBtn) {
            ...
            const scriptData = {
                ...
                title: title || '제목 없음',
                sections: this.scriptSections.map(s => ({
                    id: s.id || this.generateSectionId(),
                    type: s.type || 'custom',
                    order: s.order || 0,
                    content: s.content || '',
                ...
                totalCharCount: this.scriptSections.reduce((sum, s) => sum + (s.charCount || 0), 0),
                completionRate: this.calculateCompletionRate(),
                createdAt: this.currentScriptId ? null : firebase.firestore.FieldValue.serverTimestamp(),
                updatedAt: firebase.firestore.FieldValue.serverTimestamp()
            };

            // Firebase 트랜잭션 사용 (배치 저장으로 네트워크 요청 최소화)
            if (this.currentScriptId) {
                // 업데이트: updateScript 함수 사용
                await this.updateScript(this.currentScriptId, scriptData);
                if (this.showMessage) {
                    this.showMessage('스크립트가 수정되었습니다.', 'success');
                }
            } else {
                // 새로 생성: createScript 함수 사용
                const docRef = await this.createScript(scriptData);
                this.currentScriptId = docRef.id;
            ...
            if (error.code === 'unavailable' || error.code === 'deadline-exceeded') {
                if (this.showMessage) {
                    this.showMessage('네트워크 오류로 인해 저장할 수 없습니다. 나중에 다시 시도해주세요.', 'error');
                }
            } else if (error.code === 'permission-denied') {
                if (this.showMessage) {
                    this.showMessage('저장 권한이 없습니다. 로그인 상태를 확인해주세요.', 'error');
                }
            } else {
                if (this.showMessage) {
                    this.showMessage('스크립트를 저장하는 중 오류가 발생했습니다: ' + (error.message || '알 수 없는 오류'), 'error');
                }
            }
        } finally {
            // 로딩 상태 해제
     ...
     * - 로딩 상태 알림
     * 
     * @param {string} scriptId - 불러올 스크립트 ID
     * @throws {Error} 불러오기 실패 시 에러 로깅
     */
    async loadScript(scriptId) {
        try {
            // 입력값 검증
            if (!scriptId || typeof scriptId !== 'string') {
                console.warn('⚠️ 유효하지 않은 스크립트 ID:', scriptId);
                if (this.showMessage) {
                    this.showMessage('유효하지 않은 스크립트 ID입니다.', 'warning');
                }
                return;
            }
            ...
            // 로딩 상태 알림
            if (this.showMessage) {
                this.showMessage('스크립트를 불러오는 중...', 'info');
            }

            // Firebase에서 스크립트 불러오기 (getUserScripts 또는 직접 조회)
            const scriptDoc = await this.db.collection(DualTextWriter.SCRIPT_COLLECTION).doc(scriptId).get();

            if (!scriptDoc.exists) {
                if (this.showMessage) {
                    this.showMessage('스크립트를 찾을 수 없습니다.', 'warning');
                }
                return;
            }
            ...
            // 사용자 확인 (본인의 스크립트만 불러오기)
            if (scriptData.userId !== this.currentUser.uid) {
                if (this.showMessage) {
                    this.showMessage('이 스크립트에 접근할 권한이 없습니다.', 'error');
                }
                return;
            }
            ...
            // 제목 설정
            if (this.scriptTitleInput && scriptData.title) {
                this.scriptTitleInput.value = scriptData.title;
            }

            // 섹션 데이터 로드
            if (scriptData.sections && Array.isArray(scriptData.sections)) {
                this.scriptSections = scriptData.sections.map(section => ({
                    id: section.id || this.generateSectionId(),
                    type: section.type || 'custom',
                    order: section.order || 0,
                    content: section.content || '',
                    charCount: section.charCount || 0,
                ...
                // 순서 정렬
                this.scriptSections.sort((a, b) => a.order - b.order);
                ...
                if (this.showMessage) {
                    this.showMessage('스크립트를 불러왔습니다.', 'success');
                }
            } else {
                if (this.showMessage) {
                    this.showMessage('스크립트 데이터 형식이 올바르지 않습니다.', 'warning');
                }
            }
        } catch (error) {
            ...
            if (error.code === 'unavailable' || error.code === 'deadline-exceeded') {
                if (this.showMessage) {
                    this.showMessage('네트워크 오류로 인해 불러올 수 없습니다. 나중에 다시 시도해주세요.', 'error');
                }
            } else if (error.code === 'permission-denied') {
                if (this.showMessage) {
                    this.showMessage('스크립트를 불러올 권한이 없습니다.', 'error');
                }
            } else {
                if (this.showMessage) {
                    this.showMessage('스크립트를 불러오는 중 오류가 발생했습니다: ' + (error.message || '알 수 없는 오류'), 'error');
                }
            }
        }
    }
    ...
    initResponsiveHandlers() {
        try {
            ...
            const handleResize = () => {
                ...
                this._resizeTimer = setTimeout(() => {
                    try {
                        ...
                        // 화면 크기가 실제로 변경된 경우에만 레이아웃 재조정
                        if (Math.abs(currentWidth - this._lastWindowWidth) > 50) {
            ...
            console.error('❌ 반응형 디자인 초기화 실패:', error);
        }
    }
     ...
     * 레이아웃 재조정 (화면 크기 변경 시)
     * 
     * - 모바일/태블릿/데스크톱 레이아웃 조정
     * - 가상 키보드 대응 (모바일)
     * - 접근성 유지
     * 
     * @throws {Error} 레이아웃 재조정 실패 시 에러 로깅
     */
    handleLayoutResize() {
        try {
            ...
            // 스크립트 작성 탭이 활성화된 경우에만 레이아웃 재조정
            if (this.scriptTabContent && this.scriptTabContent.getAttribute('aria-hidden') === 'false') {
                // 모바일: 가상 키보드 대응
                if (isMobile) {
                    // textarea 포커스 시 자동 스크롤
                    const textareas = this.scriptTabContent.querySelectorAll('.script-section-content');
                ...
                if (isTablet || isDesktop) {
                    // 미리보기 섹션 자동 스크롤 (필요시)
                    if (this.scriptPreviewSection && this.scriptPreviewSection.style.display !== 'none') {
                        this.scriptPreviewSection.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
                    }
                }
            }
        } catch (error) {
            console.error('❌ 레이아웃 재조정 실패:', error);
        }
    }
     ...
     * - 가상 키보드 대응
     * 
     * @throws {Error} 모바일 최적화 실패 시 에러 로깅
     */
    optimizeMobileTouch() {
        try {
            const width = window.innerWidth;
            if (width > 768) return; // 모바일이 아닌 경우 스킵

            // 터치 영역 최소 44px 확인 및 조정
            const touchTargets = this.scriptTabContent?.querySelectorAll('.btn-icon, .btn, button');
            if (touchTargets) {
                touchTargets.forEach(target => {
                    const rect = target.getBoundingClientRect();
                    if (rect.width < 44 || rect.height < 44) {
                        // CSS에서 이미 처리되지만, 추가 확인
            ...
            // 터치 스크롤 최적화
            const scrollableContainers = this.scriptTabContent?.querySelectorAll('.script-sections-container, .script-preview-content');
            ...
            console.error('❌ 모바일 터치 최적화 실패:', error);
        }
    }
     ...
     * 섹션 순서 저장
     * 
     * - 기존 저장 로직과 일치 (Firebase 저장 패턴 재사용)
     * - 배치 업데이트로 네트워크 요청 최소화
     * - Firebase 트랜잭션 사용 (선택적)
     * - 저장 실패 시 롤백
     * - 접근성: 저장 중 로딩 상태 알림
     * 
     * @throws {Error} 저장 실패 시 에러 로깅
    ...
    async saveSectionOrder() {
        try {
            // Firebase 준비 확인
            if (!this.isFirebaseReady || !this.db) {
                console.warn('⚠️ Firebase가 준비되지 않았습니다. 순서는 로컬에만 저장됩니다.');
                return;
            }

            // 사용자 확인
            if (!this.currentUser) {
                console.warn('⚠️ 로그인이 필요합니다. 순서는 로컬에만 저장됩니다.');
                return;
            }

            // 스크립트 ID 확인
            if (!this.currentScriptId) {
                console.warn('⚠️ 저장된 스크립트가 없습니다. 순서는 로컬에만 저장됩니다.');
                return;
            }

            // 섹션 순서 배열 준비
            const sectionOrder = this.scriptSections.map(s => ({
                id: s.id,
                order: s.order
            }));

            // 접근성: 저장 중 로딩 상태 알림
            const saveButton = this.scriptSaveBtn;
            if (saveButton) {
                saveButton.setAttribute('aria-busy', 'true');
                saveButton.disabled = true;
            }

            // 저장 중 메시지 영역 (aria-live)
            let statusMessage = document.getElementById('script-save-status');
            if (!statusMessage) {
                statusMessage = document.createElement('div');
                statusMessage.id = 'script-save-status';
                statusMessage.setAttribute('role', 'status');
                statusMessage.setAttribute('aria-live', 'polite');
                statusMessage.setAttribute('aria-atomic', 'true');
                statusMessage.className = 'sr-only';
                if (this.scriptSaveBtn && this.scriptSaveBtn.parentElement) {
                    this.scriptSaveBtn.parentElement.appendChild(statusMessage);
                }
            }
            statusMessage.textContent = '섹션 순서 저장 중...';

            try {
                // Firebase 트랜잭션 사용 (데이터 일관성 보장)
                // 기존 저장 로직과 일치하는 패턴 사용
                const scriptRef = this.db.collection(DualTextWriter.SCRIPT_COLLECTION).doc(this.currentScriptId);
                ...
                await this.db.runTransaction(async (transaction) => {
                    ...
                    const currentData = scriptDoc.data();
                    const currentSections = currentData.sections || [];

                    // 섹션 순서 업데이트 (배치 업데이트로 네트워크 요청 최소화)
                    const updatedSections = currentSections.map(section => {
                        const orderInfo = sectionOrder.find(so => so.id === section.id);
                        if (orderInfo) {
                            return {
                                ...section,
                ...
                // 저장 완료 메시지
                statusMessage.textContent = '섹션 순서가 저장되었습니다.';
                if (this.showMessage) {
                    // 시각적 메시지는 표시하지 않음 (자동 저장이므로)
                    // 필요시 주석 해제: this.showMessage('섹션 순서가 저장되었습니다.', 'success');
                }

                console.log('✅ 섹션 순서 저장 완료');
            } catch (error) {
                // 트랜잭션 실패 시 롤백 (로컬 상태는 이미 변경됨)
                console.error('❌ 섹션 순서 저장 실패:', error);

                // 에러 메시지
                statusMessage.textContent = '섹션 순서 저장 실패: ' + (error.message || '알 수 없는 오류');
                
                // 네트워크 오류 처리
                if (error.code === 'unavailable' || error.code === 'deadline-exceeded') {
                    if (this.showMessage) {
                        this.showMessage('네트워크 오류로 인해 섹션 순서를 저장할 수 없습니다. 나중에 다시 시도해주세요.', 'warning');
                    }
                } else {
                    if (this.showMessage) {
                        this.showMessage('섹션 순서를 저장하는 중 오류가 발생했습니다: ' + (error.message || '알 수 없는 오류'), 'error');
                    }
                }

                // 롤백: 로컬 상태를 이전 상태로 복구 (선택적)
                // 주의: 이미 UI가 업데이트되었으므로, 사용자에게 알림만 제공
                throw error;
            } finally {
                // 접근성: 저장 완료 후 로딩 상태 해제
                if (saveButton) {
            ...
            console.error('❌ 섹션 순서 저장 실패:', error);
            // 최상위 에러 처리 (트랜잭션 외부 에러)
            if (this.showMessage && !error.message) {
                this.showMessage('섹션 순서를 저장하는 중 오류가 발생했습니다.', 'error');
            }
        }
    }
     ...
     * 
     * - 클릭 이벤트 처리
     * - 키보드 네비게이션 지원 (화살표 키, Home, End)
     * 
     * @throws {Error} 이벤트 리스너 바인딩 실패 시 에러 로깅
     */
    initTabListeners() {
        try {
            this.tabButtons.forEach((button, index) => {
                ...
                button.addEventListener('keydown', (e) => {
                    const tabButtonsArray = Array.from(this.tabButtons);
                    const currentIndex = tabButtonsArray.indexOf(button);
                    ...
                    switch (e.key) {
                        ...
                        case 'ArrowDown':
                            e.preventDefault();
                            targetIndex = (currentIndex + 1) % tabButtonsArray.length;
                            break;
                        case 'ArrowLeft':
                        case 'ArrowUp':
                            e.preventDefault();
                            targetIndex = (currentIndex - 1 + tabButtonsArray.length) % tabButtonsArray.length;
                            break;
                        case 'Home':
                            e.preventDefault();
                            targetIndex = 0;
                        ...
                        case 'End':
                            e.preventDefault();
                            targetIndex = tabButtonsArray.length - 1;
                            break;
                        case 'Enter':
                        case ' ':
                            e.preventDefault();
                    ...
                    // 포커스 이동
                    if (targetIndex >= 0 && targetIndex < tabButtonsArray.length) {
                        tabButtonsArray[targetIndex].focus();
                    }
                });
            });
        } catch (error) {
            console.error('❌ 탭 리스너 초기화 실패:', error);
        }
    }

    // 탭 전환
    switchTab(tabName) {
        // 모든 탭 버튼과 콘텐츠에서 active 클래스 제거 및 접근성 속성 업데이트
        ...
        // 선택된 탭 버튼과 콘텐츠에 active 클래스 추가 및 접근성 속성 업데이트
        ...
        // 스크립트 작성 탭으로 전환 시 초기화
        if (tabName === 'script') {
            this.initScriptWritingTab();
        }
    }
...
현재는 로컬 스토리지 모드로 동작합니다.
     ...
     */
    testScriptWritingFeatures() {
        const results = {
            edgeCases: { passed: 0, failed: 0, tests: [] },
            errorScenarios: { passed: 0, failed: 0, tests: [] },
            performance: { passed: false, duration: 0 },
            memory: { passed: false, leaks: [] }
        };
        ...
        try {
            ...
            try {
                this.renderScriptSections();
                this.updateCompletionRate();
                results.edgeCases.tests.push({ name: '빈 섹션 배열 처리', passed: true });
                results.edgeCases.passed++;
            } catch (error) {
                results.edgeCases.tests.push({ name: '빈 섹션 배열 처리', passed: false, error: error.message });
                results.edgeCases.failed++;
            }

            // 1.2 최대 섹션 수 초과 테스트
            this.scriptSections = [];
            for (let i = 0; i < DualTextWriter.MAX_SECTIONS + 5; i++) {
                this.scriptSections.push(this.createScriptSection('custom', i + 1, `섹션 ${i + 1}`));
            }
            try {
                const beforeCount = this.scriptSections.length;
                this.addNewSection();
                const afterCount = this.scriptSections.length;
                const passed = afterCount <= DualTextWriter.MAX_SECTIONS;
                results.edgeCases.tests.push({ 
                    name: '최대 섹션 수 초과 방지', 
                    passed: passed,
                    before: beforeCount,
                    after: afterCount
                ...
                else results.edgeCases.failed++;
            } catch (error) {
                results.edgeCases.tests.push({ name: '최대 섹션 수 초과 방지', passed: false, error: error.message });
                results.edgeCases.failed++;
            }

            // 1.3 최소 섹션 수 미만 삭제 방지 테스트
            this.scriptSections = [this.createScriptSection('introduction', 1, '테스트')];
            try {
                const beforeCount = this.scriptSections.length;
                // 최소 섹션 수 체크는 deleteSection 내부에서 처리되므로, 직접 호출하지 않고 로직 확인
                const passed = this.scriptSections.length <= DualTextWriter.MIN_SECTIONS;
                results.edgeCases.tests.push({ 
                    name: '최소 섹션 수 미만 삭제 방지', 
                    passed: passed,
                    before: beforeCount,
                    note: 'deleteSection 함수 내부에서 MIN_SECTIONS 체크 수행'
                });
                if (passed) results.edgeCases.passed++;
                else results.edgeCases.failed++;
            } catch (error) {
                results.edgeCases.tests.push({ name: '최소 섹션 수 미만 삭제 방지', passed: false, error: error.message });
                results.edgeCases.failed++;
            }

            // 1.4 매우 긴 텍스트 테스트
            const longText = '가'.repeat(DualTextWriter.SECTION_MAX_CHAR_COUNT + 100);
            try {
                const section = this.createScriptSection('custom', 1, longText);
                const passed = section.content.length <= DualTextWriter.SECTION_MAX_CHAR_COUNT;
                results.edgeCases.tests.push({ 
                    name: '최대 글자 수 제한', 
                    passed: passed,
                ...
                results.edgeCases.tests.push({ name: '최대 글자 수 제한', passed: false, error: error.message });
                results.edgeCases.failed++;
            }
            ...
            // 2.1 Firebase 미준비 상태 테스트
            const originalFirebaseReady = this.isFirebaseReady;
            this.isFirebaseReady = false;
            try {
                await this.saveScript();
                results.errorScenarios.tests.push({ name: 'Firebase 미준비 상태 처리', passed: true });
                results.errorScenarios.passed++;
            } catch (error) {
                results.errorScenarios.tests.push({ name: 'Firebase 미준비 상태 처리', passed: false, error: error.message });
                results.errorScenarios.failed++;
            } finally {
                this.isFirebaseReady = originalFirebaseReady;
            }
            ...
            try {
                await this.saveScript();
                results.errorScenarios.tests.push({ name: '사용자 미로그인 상태 처리', passed: true });
                results.errorScenarios.passed++;
            } catch (error) {
                results.errorScenarios.tests.push({ name: '사용자 미로그인 상태 처리', passed: false, error: error.message });
                results.errorScenarios.failed++;
            } finally {
            ...
            // 2.3 잘못된 섹션 ID 테스트
            try {
                await this.deleteSection('invalid-section-id');
                results.errorScenarios.tests.push({ name: '잘못된 섹션 ID 처리', passed: true });
                results.errorScenarios.passed++;
            } catch (error) {
                results.errorScenarios.tests.push({ name: '잘못된 섹션 ID 처리', passed: false, error: error.message });
                results.errorScenarios.failed++;
            }
            ...
            this.scriptSections = [];
            for (let i = 0; i < 100; i++) {
                this.scriptSections.push(this.createScriptSection('custom', i + 1, `섹션 ${i + 1} 내용`));
            }
            ...
            results.performance.passed = duration < 1000; // 1초 이하
            
            console.log(`성능 테스트: ${duration.toFixed(2)}ms (${results.performance.passed ? '✅ 통과' : '❌ 실패'})`);

            // 4. 메모리 누수 확인
            console.log('🧪 메모리 누수 확인 시작...');
            ...
            // 메모리 증가가 10MB 이하인 경우 통과
            results.memory.passed = memoryIncrease < 10 * 1024 * 1024;
            if (memoryIncrease > 10 * 1024 * 1024) {
                results.memory.leaks.push(`메모리 증가: ${(memoryIncrease / 1024 / 1024).toFixed(2)}MB`);
            }
            
            console.log(`메모리 테스트: ${(memoryIncrease / 1024 / 1024).toFixed(2)}MB 증가 (${results.memory.passed ? '✅ 통과' : '❌ 실패'})`);

            // 테스트 결과 요약
            console.log('📊 테스트 결과 요약:');
            console.log(`엣지 케이스: ${results.edgeCases.passed}/${results.edgeCases.passed + results.edgeCases.failed} 통과`);
            console.log(`에러 시나리오: ${results.errorScenarios.passed}/${results.errorScenarios.passed + results.errorScenarios.failed} 통과`);
            console.log(`성능: ${results.performance.passed ? '✅ 통과' : '❌ 실패'} (${results.performance.duration.toFixed(2)}ms)`);
            console.log(`메모리: ${results.memory.passed ? '✅ 통과' : '❌ 실패'}`);
     ...
     */
    testScriptWritingAccessibility() {
        const results = {
            screenReader: { passed: 0, failed: 0, tests: [] },
            keyboard: { passed: 0, failed: 0, tests: [] }
        };

        try {
            // 1. 스크린 리더 테스트 (ARIA 속성 확인)
            console.log('♿ 스크린 리더 테스트 시작...');

            // 1.1 탭 버튼 ARIA 속성
            const scriptTab = this.scriptTab;
            if (scriptTab) {
                const hasRole = scriptTab.getAttribute('role') === 'tab';
                const hasAriaLabel = scriptTab.hasAttribute('aria-label');
                const hasAriaControls = scriptTab.hasAttribute('aria-controls');
                const passed = hasRole && hasAriaLabel && hasAriaControls;
                results.screenReader.tests.push({ 
                    name: '탭 버튼 ARIA 속성', 
                    passed: passed,
                    details: { hasRole, hasAriaLabel, hasAriaControls }
                });
                if (passed) results.screenReader.passed++;
                else results.screenReader.failed++;
            }

            // 1.2 섹션 콘텐츠 ARIA 속성
            const sections = this.scriptTabContent?.querySelectorAll('.script-section');
            if (sections && sections.length > 0) {
                const firstSection = sections[0];
                const textarea = firstSection.querySelector('.script-section-content');
                if (textarea) {
                    const hasAriaLabel = textarea.hasAttribute('aria-label');
                    const hasAriaDescribedBy = textarea.hasAttribute('aria-describedby');
                    const passed = hasAriaLabel && hasAriaDescribedBy;
                    results.screenReader.tests.push({ 
                        name: '섹션 콘텐츠 ARIA 속성', 
                        passed: passed,
                        details: { hasAriaLabel, hasAriaDescribedBy }
                    });
                    if (passed) results.screenReader.passed++;
                    else results.screenReader.failed++;
                }
            ...
            const progressBar = document.getElementById('script-completion-progressbar');
            if (progressBar) {
                const hasRole = progressBar.getAttribute('role') === 'progressbar';
                const hasAriaValueNow = progressBar.hasAttribute('aria-valuenow');
                const hasAriaLabel = progressBar.hasAttribute('aria-label');
                const passed = hasRole && hasAriaValueNow && hasAriaLabel;
                results.screenReader.tests.push({ 
                    name: '진행률 바 ARIA 속성', 
                    passed: passed,
                    details: { hasRole, hasAriaValueNow, hasAriaLabel }
                });
                if (passed) results.screenReader.passed++;
                else results.screenReader.failed++;
            }
            ...
            // 2.1 모든 버튼이 키보드로 접근 가능한지 확인
            const buttons = this.scriptTabContent?.querySelectorAll('button, [role="button"]');
            if (buttons) {
                ...
                buttons.forEach(button => {
                    ...
                    const isAccessible = (tabindex === null || tabindex !== '-1') && !disabled;
                    
                    if (isAccessible) accessibleCount++;
                    else inaccessibleCount++;
                });
                
                const passed = inaccessibleCount === 0;
                results.keyboard.tests.push({ 
                    name: '모든 버튼 키보드 접근 가능', 
                    passed: passed,
                    details: { accessible: accessibleCount, inaccessible: inaccessibleCount }
                });
                if (passed) results.keyboard.passed++;
                else results.keyboard.failed++;
            }

            // 2.2 포커스 스타일 확인
            const focusableElements = this.scriptTabContent?.querySelectorAll('button, input, textarea, [tabindex]:not([tabindex="-1"])');
            if (focusableElements && focusableElements.length > 0) {
                const firstElement = focusableElements[0];
                const computedStyle = window.getComputedStyle(firstElement, ':focus-visible');
                const hasOutline = computedStyle.outlineWidth !== '0px' || computedStyle.outline !== 'none';
                const passed = hasOutline;
                results.keyboard.tests.push({ 
                    name: '포커스 스타일 존재', 
                    passed: passed
                });
            ...
            // 테스트 결과 요약
            console.log('📊 접근성 테스트 결과 요약:');
            console.log(`스크린 리더: ${results.screenReader.passed}/${results.screenReader.passed + results.screenReader.failed} 통과`);
            console.log(`키보드 네비게이션: ${results.keyboard.passed}/${results.keyboard.passed + results.keyboard.failed} 통과`);
     ...
     * - 전체 시스템 성능 테스트
     * 
     * @returns {Object} 통합 테스트 결과
     */
    async testScriptWritingIntegration() {
        const results = {
            workflow: { passed: 0, failed: 0, tests: [] },
            failureScenarios: { passed: 0, failed: 0, tests: [] },
        ...
        try {
            ...
            // 1.1 스크립트 생성 → 섹션 추가 → 내용 입력 → 저장
            try {
                // 초기화
                this.scriptSections = [];
                this.currentScriptId = null;
                if (this.scriptTitleInput) {
                    this.scriptTitleInput.value = '통합 테스트 스크립트';
                }

                // 기본 섹션 생성
                this.createDefaultSections();
                const passed1 = this.scriptSections.length === 3;
                results.workflow.tests.push({ name: '기본 섹션 생성', passed: passed1 });
                if (passed1) results.workflow.passed++;
                else results.workflow.failed++;

                // 섹션 내용 입력
                if (this.scriptSections.length > 0) {
                    this.scriptSections[0].content = '서론 내용';
                    this.scriptSections[1].content = '본론 내용';
                    this.scriptSections[2].content = '결론 내용';
                }

                // 섹션 렌더링
                this.renderScriptSections();
                const passed2 = this.scriptSectionsContainer?.children.length === 3;
                results.workflow.tests.push({ name: '섹션 렌더링', passed: passed2 });
                if (passed2) results.workflow.passed++;
                else results.workflow.failed++;

                // 완료율 계산
                this.updateCompletionRate();
                const completionRate = this.calculateCompletionRate();
                const passed3 = typeof completionRate === 'number' && completionRate >= 0 && completionRate <= 100;
                results.workflow.tests.push({ name: '완료율 계산', passed: passed3 });
                if (passed3) results.workflow.passed++;
                else results.workflow.failed++;

                // 미리보기 생성
                this.renderFullPreview();
                const passed4 = this.scriptPreviewContent?.innerHTML.length > 0;
                results.workflow.tests.push({ name: '미리보기 생성', passed: passed4 });
                if (passed4) results.workflow.passed++;
                else results.workflow.failed++;

            } catch (error) {
                results.workflow.tests.push({ name: '전체 워크플로우', passed: false, error: error.message });
                results.workflow.failed++;
            }

            // 2. 실패 시나리오 테스트
            console.log('⚠️ 실패 시나리오 테스트 시작...');
            ...
            try {
                await this.saveScript();
                results.failureScenarios.tests.push({ name: '네트워크 오류 처리', passed: true });
                results.failureScenarios.passed++;
            } catch (error) {
                results.failureScenarios.tests.push({ name: '네트워크 오류 처리', passed: false, error: error.message });
                results.failureScenarios.failed++;
            } finally {
                this.db = originalDb;
            }
            ...
            // 2.2 잘못된 데이터 형식 처리
            try {
                this.scriptSections = null;
                this.renderScriptSections();
                results.failureScenarios.tests.push({ name: '잘못된 데이터 형식 처리', passed: true });
                results.failureScenarios.passed++;
            } catch (error) {
                results.failureScenarios.tests.push({ name: '잘못된 데이터 형식 처리', passed: false, error: error.message });
                results.failureScenarios.failed++;
            } finally {
                this.scriptSections = [];
            }
            ...
            // 3. 전체 시스템 성능 테스트
            console.log('⚡ 전체 시스템 성능 테스트 시작...');
            const startTime = performance.now();

            // 대량 데이터 처리
            this.scriptSections = [];
            for (let i = 0; i < 50; i++) {
                this.scriptSections.push(this.createScriptSection('custom', i + 1, `섹션 ${i + 1} 내용`.repeat(10)));
            }
            ...
            results.performance.metrics = {
                duration: duration,
                sectionsCount: this.scriptSections.length,
                averageTimePerSection: duration / this.scriptSections.length
            };
            results.performance.passed = duration < 2000; // 2초 이하

            console.log(`전체 시스템 성능: ${duration.toFixed(2)}ms (${results.performance.passed ? '✅ 통과' : '❌ 실패'})`);

            // 테스트 결과 요약
            console.log('📊 통합 테스트 결과 요약:');
            console.log(`워크플로우: ${results.workflow.passed}/${results.workflow.passed + results.workflow.failed} 통과`);
            console.log(`실패 시나리오: ${results.failureScenarios.passed}/${results.failureScenarios.passed + results.failureScenarios.failed} 통과`);
            console.log(`성능: ${results.performance.passed ? '✅ 통과' : '❌ 실패'} (${duration.toFixed(2)}ms)`);
     ...
     * - 접근성 고려
     * 
     * @returns {Object} 코드 품질 검증 결과
     */
    validateCodeQuality() {
        const results = {
            codingStyle: { passed: 0, failed: 0, issues: [] },
        ...
        try {
            // 1. 코딩 스타일 일치 여부 검증
            console.log('🔍 코딩 스타일 검증 시작...');

            // 1.1 들여쓰기 일관성 (코드 검토로 확인, 자동화 어려움)
            results.codingStyle.issues.push({ 
                item: '들여쓰기 일관성', 
                status: 'manual-review',
                note: '코드 리뷰로 확인 필요 (스페이스 4개 사용)'
            });

            // 1.2 네이밍 컨벤션 (camelCase 확인)
            ...
            scriptFunctions.forEach(funcName => {
                if (typeof this[funcName] !== 'function') {
                    namingPassed = false;
                    results.codingStyle.issues.push({ 
                        item: `함수 ${funcName} 존재 확인`, 
                        status: 'failed',
                        note: '함수가 정의되지 않음'
                    });
                }
            ...
            if (namingPassed) {
                results.codingStyle.passed++;
            } else {
                results.codingStyle.failed++;
            }

            // 1.3 JSDoc 주석 확인
            const hasJSDoc = this.initScriptWritingTab.toString().includes('@throws') ||
                            this.initScriptWritingTab.toString().includes('@param');
            if (hasJSDoc) {
                results.codingStyle.passed++;
            } else {
                results.codingStyle.failed++;
                results.codingStyle.issues.push({ 
                    item: 'JSDoc 주석', 
                    status: 'warning',
                    note: '일부 함수에 JSDoc 주석 추가 권장'
                });
            }

            // 2. 명확한 함수명/변수명 검증
            console.log('📝 함수명/변수명 검증 시작...');

            // 2.1 함수명이 동작을 명확히 표현하는지 확인
            const functionNames = [
                { name: 'initScriptWritingTab', expected: '초기화' },
                { name: 'createDefaultSections', expected: '생성' },
                { name: 'renderScriptSections', expected: '렌더링' },
                { name: 'addNewSection', expected: '추가' },
                { name: 'deleteSection', expected: '삭제' },
                { name: 'updateSectionContent', expected: '업데이트' }
            ];

            functionNames.forEach(({ name, expected }) => {
                if (typeof this[name] === 'function') {
                    const funcNameLower = name.toLowerCase();
                    if (funcNameLower.includes(expected.toLowerCase()) || 
                        funcNameLower.includes('script') || 
                        funcNameLower.includes('section')) {
                        ...
                        results.naming.issues.push({ 
                            item: `함수명 ${name}`, 
                            status: 'warning',
                            note: '함수명이 동작을 명확히 표현하는지 확인 필요'
                        });
                    }
                }
            });

            // 2.2 배열 변수는 복수형 사용 확인
            const arrayVariables = ['scriptSections'];
            arrayVariables.forEach(varName => {
                if (this[varName] && Array.isArray(this[varName])) {
                    if (varName.endsWith('s') || varName.includes('List') || varName.includes('Array')) {
                        results.naming.passed++;
                    } else {
                        results.naming.failed++;
                        results.naming.issues.push({ 
            ...
            // 3. 에러 처리 존재 여부 검증
            console.log('⚠️ 에러 처리 검증 시작...');

            // 3.1 비동기 함수에 try-catch 확인
            const asyncFunctions = ['saveScript', 'loadScript', 'deleteScript', 'createScript', 'updateScript'];
            asyncFunctions.forEach(funcName => {
                if (typeof this[funcName] === 'function') {
                    const funcString = this[funcName].toString();
                    if (funcString.includes('try') && funcString.includes('catch')) {
                        results.errorHandling.passed++;
                    } else {
                        results.errorHandling.failed++;
                        results.errorHandling.issues.push({ 
                            item: `비동기 함수 ${funcName}`, 
                            status: 'failed',
                            note: 'try-catch 블록이 없습니다'
                        });
                    }
                }
            });
            ...
            // 3.2 입력값 검증 확인
            const validationFunctions = ['addNewSection', 'deleteSection', 'updateSectionContent'];
            validationFunctions.forEach(funcName => {
                if (typeof this[funcName] === 'function') {
                    const funcString = this[funcName].toString();
                    if (funcString.includes('if') && (funcString.includes('!') || funcString.includes('length'))) {
                        results.errorHandling.passed++;
                    } else {
                        results.errorHandling.failed++;
                        results.errorHandling.issues.push({ 
                            item: `함수 ${funcName} 입력값 검증`, 
                            status: 'warning',
                            note: '입력값 검증 추가 권장'
                        });
                    }
            ...
            // 4.1 이벤트 리스너 중복 방지 확인
            if (this._scriptEventBound !== undefined) {
                ...
                results.performance.issues.push({ 
                    item: '이벤트 리스너 중복 방지', 
                    status: 'warning',
                    note: '이벤트 리스너 중복 방지 플래그 확인 필요'
                });
            }

            // 4.2 디바운싱 적용 확인
            const debounceFunctions = ['updateSectionContent', 'saveSectionCompleteState'];
            debounceFunctions.forEach(funcName => {
                if (typeof this[funcName] === 'function') {
                    const funcString = this[funcName].toString();
                    if (funcString.includes('setTimeout') || funcString.includes('debounce') || funcString.includes('Timer')) {
                        results.performance.passed++;
                    } else {
                        results.performance.failed++;
                        results.performance.issues.push({ 
                            item: `함수 ${funcName} 디바운싱`, 
                            status: 'warning',
                            note: '디바운싱 적용 권장'
                        });
                    }
            ...
            // 4.3 DOM 조작 최소화 확인
            const renderFunctions = ['renderScriptSections'];
            renderFunctions.forEach(funcName => {
                if (typeof this[funcName] === 'function') {
                    const funcString = this[funcName].toString();
                    const innerHTMLCount = (funcString.match(/innerHTML/g) || []).length;
                    if (innerHTMLCount <= 2) { // 한 번의 innerHTML 할당이 이상적
                        results.performance.passed++;
                    } else {
                        results.performance.failed++;
                        results.performance.issues.push({ 
                            item: `함수 ${funcName} DOM 조작`, 
                            status: 'warning',
                            note: `innerHTML 사용 ${innerHTMLCount}회, 최소화 권장`
                        });
                    }
                }
            });
            ...
            // 5.1 모든 인터랙티브 요소에 aria-label 확인
            const interactiveElements = this.scriptTabContent?.querySelectorAll('button, input, textarea, [role="button"]');
            if (interactiveElements) {
                ...
                const ariaLabelRatio = totalCount > 0 ? (ariaLabelCount / totalCount) : 0;
                if (ariaLabelRatio >= 0.9) { // 90% 이상
                    ...
                    results.accessibility.issues.push({ 
                        item: 'aria-label 속성', 
                        status: 'warning',
                        note: `${ariaLabelCount}/${totalCount} 요소에 aria-label 있음 (${(ariaLabelRatio * 100).toFixed(1)}%)`
                    });
                }
            }
            ...
            if (focusableElements && focusableElements.length > 0) {
                ...
                focusableElements.forEach(element => {
                    ...
                    if ((tabindex === null || tabindex !== '-1') && !disabled) {
                        keyboardAccessibleCount++;
                    }
                });

                const keyboardRatio = focusableElements.length > 0 ? 
                    (keyboardAccessibleCount / focusableElements.length) : 0;
                if (keyboardRatio >= 0.9) {
                    results.accessibility.passed++;
                } else {
                    results.accessibility.failed++;
                    results.accessibility.issues.push({ 
                        item: '키보드 네비게이션', 
                        status: 'warning',
                        note: `${keyboardAccessibleCount}/${focusableElements.length} 요소가 키보드 접근 가능 (${(keyboardRatio * 100).toFixed(1)}%)`
                    });
                }
            }

            // 5.3 포커스 스타일 확인
            const styleSheet = document.styleSheet || document.querySelector('style');
            if (styleSheet) {
                // CSS에서 :focus-visible 스타일 확인 (간접적)
                results.accessibility.passed++;
            } else {
                results.accessibility.failed++;
                results.accessibility.issues.push({ 
                    item: '포커스 스타일', 
                    status: 'warning',
                    note: '포커스 스타일 CSS 확인 필요'
                });
            }

            // 검증 결과 요약
            console.log('📊 코드 품질 검증 결과 요약:');
            console.log(`코딩 스타일: ${results.codingStyle.passed}/${results.codingStyle.passed + results.codingStyle.failed} 통과`);
            console.log(`함수명/변수명: ${results.naming.passed}/${results.naming.passed + results.naming.failed} 통과`);
            console.log(`에러 처리: ${results.errorHandling.passed}/${results.errorHandling.passed + results.errorHandling.failed} 통과`);
            console.log(`성능: ${results.performance.passed}/${results.performance.passed + results.performance.failed} 통과`);
            console.log(`접근성: ${results.accessibility.passed}/${results.accessibility.passed + results.accessibility.failed} 통과`);

            // 이슈가 있는 경우 출력
            const allIssues = [
                ...results.codingStyle.issues,
                ...results.naming.issues,
                ...results.errorHandling.issues,
                ...results.performance.issues,
                ...results.accessibility.issues
            ].filter(issue => issue.status === 'failed' || issue.status === 'warning');

            if (allIssues.length > 0) {
                console.log('\n⚠️ 발견된 이슈:');
                allIssues.forEach(issue => {
                    console.log(`  - ${issue.item}: ${issue.note}`);
                });
            }

        } catch (error) {
            console.error('❌ 코드 품질 검증 실패:', error);
        }
