...
class DualTextWriter {
    ...
    showInlineDuplicateHint(duplicate) {
        ...
        const topicStr = duplicate?.topic ? ` · 주제: ${escapeHtml(duplicate.topic)}` : '';
    ...
    initReferenceLoader() {
        ...
        this.referenceLoaderCloseBtn = document.getElementById('reference-loader-close');
        ...
        if (this.referenceLoaderPanel) {
            this.referenceLoaderPanel.addEventListener('click', (e) => {
                if (e.target === this.referenceLoaderPanel || e.target.classList.contains('reference-loader-overlay')) {
                    this.closeReferenceLoader();
                }
            });
        }
        ...
        if (this.referenceSavedList) {
            this.referenceSavedList.addEventListener('click', (e) => this.handleReferenceItemClick(e));
        }
        if (this.referenceTrackingList) {
            this.referenceTrackingList.addEventListener('click', (e) => this.handleReferenceItemClick(e));
        }
        
        // 이벤트 리스너: 검색
        if (this.referenceSearchInput) {
            this.referenceSearchInput.addEventListener('input', debounce(() => {
    ...
    switchReferenceLoaderTab(tabName) {
        ...
        if (tabName === 'saved') {
            ...
            document.getElementById('reference-tracking-filters').style.display = 'none';
        } else {
            this.referenceSavedContent.style.display = 'none';
            this.referenceTrackingContent.style.display = 'block';
            document.getElementById('reference-tracking-filters').style.display = 'flex';
        }
    }
    ...
    async loadSavedReferencesForLoader() {
        ...
        try {
            ...
            this.renderReferenceLoaderList(this.savedTexts, this.referenceSavedList, 'saved');
        } catch (error) {
    ...
    renderReferenceLoaderList(items, container, sourceType) {
        ...
        items.forEach(item => {
            ...
            const contentPreview = item.content ? item.content.replace(/<[^>]*>/g, '').substring(0, 100) + (item.content.length > 100 ? '...' : '') : '';
            
            el.innerHTML = `
                <div class="reference-item-header">
                    <div class="reference-item-title">${escapeHtml(item.topic || '제목 없음')}</div>
                </div>
                <div class="reference-item-content">${escapeHtml(contentPreview)}</div>
    ...
    addReferenceToDetail(item) {
        ...
        // 중복 체크
        const existing = this.detailReferenceList.querySelector(`[data-ref-id="${item.id}"]`);
        if (existing) {
        ...
        const contentPreview = item.content ? item.content.replace(/<[^>]*>/g, '').substring(0, 150) + (item.content.length > 150 ? '...' : '') : '';

        el.innerHTML = `
            <div class="detail-ref-header">
                <span class="detail-ref-title">${escapeHtml(item.topic || '제목 없음')}</span>
        ...
        el.querySelector('.detail-ref-remove').addEventListener('click', () => {
            ...
            if (this.detailReferenceList.children.length === 0) {
                if (this.detailReferenceEmpty) this.detailReferenceEmpty.style.display = 'block';
    ...
    filterReferenceLoaderList() {
        const keyword = this.referenceSearchInput.value.toLowerCase();
        ...
        items.forEach(item => {
            ...
            const content = item.querySelector('.reference-item-content').textContent.toLowerCase();
...
현재는 로컬 스토리지 모드로 동작합니다.
    ...
    initArticleManagement() {
        ...
        this.managementEmptyState = document.getElementById('management-empty-state');
    ...
    async initializeArticleOrders() {
        ...
        const needsUpdate = this.managementArticles.some(article => article.order === undefined || article.order === null);
            ...
            for (const [category, articles] of Object.entries(articlesByCategory)) {
                ...
                for (let i = 0; i < articles.length; i++) {
                    ...
                    if (article.order === undefined || article.order === null) {
                        article.order = i + 1;
                        // Firebase 업데이트
                        const articleRef = window.firebaseDoc(this.db, 'users', this.currentUser.uid, 'texts', article.id);
    ...
    renderArticleCards(filterCategory = '') {
        ...
        if (filterCategory) {
            filteredArticles = this.managementArticles.filter(article => 
                (article.category || '미분류') === filterCategory
            );
        }
        ...
        Object.keys(articlesByCategory).forEach(category => {
            articlesByCategory[category].forEach(article => {
                const card = this.createArticleCard(article, globalOrder++, filterCategory);
        ...
        card.innerHTML = `
            <div class="article-card-header">
                <div class="article-card-order">
                    ...
                    <h4 class="article-card-title" title="${this.escapeHtml(article.title)}">${this.escapeHtml(article.title)}</h4>
                </div>
            ...
            <div class="article-card-content">${this.escapeHtml(contentPreview)}</div>
            <div class="article-card-meta">
                <span class="article-card-date">📅 ${dateStr}</span>
                <span class="article-card-category">📁 ${this.escapeHtml(article.category || '미분류')}</span>
            </div>
        `;
    ...
    canMoveUp(article, filterCategory = '') {
        const filtered = filterCategory 
            ? this.managementArticles.filter(a => (a.category || '미분류') === filterCategory)
            : this.managementArticles;
        
        const sameCategory = filtered.filter(a => (a.category || '미분류') === (article.category || '미분류'));
    ...
    async saveArticleEdit() {
        if (!this.selectedArticleId || !this.currentUser || !this.isFirebaseReady) return;
    ...
    async deleteArticle() {
        ...
        try {
            ...
            // 로컬 데이터에서 제거
            this.managementArticles = this.managementArticles.filter(a => a.id !== this.selectedArticleId);
    ...
    async moveArticleOrder(articleId, direction) {
        ...
        try {
            ...
            const currentIndex = sameCategoryArticles.findIndex(a => a.id === articleId);
            ...
            // 순서 교환
            const articleRef = window.firebaseDoc(this.db, 'users', this.currentUser.uid, 'texts', articleId);
            const targetRef = window.firebaseDoc(this.db, 'users', this.currentUser.uid, 'texts', targetArticle.id);
