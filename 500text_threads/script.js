...
class DualTextWriter {
  ...
  // ============================================================================
  // [Phase 10-02] DualTextWriter Constructor
  //
  // 구조 요약:
  // 1. 기본 상태 초기화 (Firebase, 사용자 설정)
  // 2. Manager 인스턴스 생성 (11개 Manager)
  // 3. 프로퍼티 위임 (Object.defineProperty - API 호환성 유지)
  // 4. DOM 요소 참조
  // 5. 초기화 호출 (init())
  //
  // Manager 목록:
  // - TrackingManager: 트래킹/메트릭 관리
  // - ReferenceManager: 레퍼런스 선택/관리
  // - SNSManager: SNS 플랫폼 관리
  // - SavedTextsManager: 저장된 글 관리
  // - ModalManager: 모달/바텀시트 관리
  // - ExpandModeManager: 확대 모드 관리
  // - TextCrudManager: 글 CRUD 관리
  // - FilterManager: 필터/검색 상태 관리
  // - LLMValidationManager: LLM 검증 관리
  // - UIManager: UI 업데이트 관리
  // - AuthManager: 인증 관리
  // - DataManager: 데이터 영속성 관리
  // - InitManager: 초기화/이벤트 바인딩 관리
  // ============================================================================
  constructor() {
    // ========================================================================
    // SECTION 1: 기본 상태 초기화
    ...
    // ========================================================================
    // SECTION 2: Manager 인스턴스 생성
    // ========================================================================

    // ==================== TrackingManager 인스턴스 생성 ====================
    // [Refactoring] 트래킹 관련 상태를 TrackingManager로 위임
    this.trackingManager = new TrackingManager(this);

    // 트래킹 관련 속성 (TrackingManager와 동기화를 위한 getter/setter)
    // 기존 코드 호환성을 위해 프로퍼티 유지 (TrackingManager로 위임)
    Object.defineProperty(this, "trackingPosts", {
      get: () => this.trackingManager.getTrackingPosts(),
      set: (value) => this.trackingManager.setTrackingPosts(value),
    });
    Object.defineProperty(this, "trackingChart", {
      get: () => this.trackingManager.getTrackingChart(),
      set: (value) => this.trackingManager.setTrackingChart(value),
    });
    Object.defineProperty(this, "currentTrackingPost", {
      get: () => this.trackingManager.getCurrentTrackingPost(),
      set: (value) => this.trackingManager.setCurrentTrackingPost(value),
    });
    Object.defineProperty(this, "chartMode", {
      get: () => this.trackingManager.getChartMode(),
      set: (value) => this.trackingManager.setChartMode(value),
    });
    Object.defineProperty(this, "selectedChartPostId", {
      get: () => this.trackingManager.getSelectedChartPostId(),
      set: (value) => this.trackingManager.setSelectedChartPostId(value),
    });
    Object.defineProperty(this, "allTrackingPostsForSelector", {
      get: () => this.trackingManager.getAllTrackingPostsForSelector(),
      set: (value) => this.trackingManager.setAllTrackingPostsForSelector(value),
    });
    Object.defineProperty(this, "chartRange", {
      get: () => this.trackingManager.getChartRange(),
      set: (value) => this.trackingManager.setChartRange(value),
    });
    Object.defineProperty(this, "scaleMode", {
      get: () => this.trackingManager.getScaleMode(),
      set: (value) => this.trackingManager.setScaleMode(value),
    });
    Object.defineProperty(this, "isBatchSelectMode", {
      get: () => this.trackingManager.getIsBatchSelectMode(),
      set: (value) => this.trackingManager.setIsBatchSelectMode(value),
    });
    Object.defineProperty(this, "selectedMetricIndices", {
      get: () => this.trackingManager.getSelectedMetricIndices(),
      set: (value) => this.trackingManager.setSelectedMetricIndices(value),
    });
    Object.defineProperty(this, "editingMetricData", {
      get: () => this.trackingManager.editingMetricData,
      set: (value) => { this.trackingManager.editingMetricData = value; },
    });
    ...
    // ==================== ExpandModeManager 인스턴스 생성 (Phase 7) ====================
    // [Refactoring] 확대 모드 관련 기능을 ExpandModeManager로 위임
    this.expandModeManager = new ExpandModeManager(this);

    // ==================== TextCrudManager 인스턴스 생성 (Phase 8) ====================
    // [Refactoring] 글 관리 (CRUD) 관련 기능을 TextCrudManager로 위임
    this.textCrudManager = new TextCrudManager(this);
    ...
    // 기존 코드에서 this.xxx로 접근하는 프로퍼티들을 Manager로 위임
    // ========================================================================

    // 확대 모드 관련 프로퍼티 (ExpandModeManager와 동기화를 위한 getter/setter)
    // 기존 코드 호환성을 위해 프로퍼티 유지 (ExpandModeManager로 위임)
    Object.defineProperty(this, "expandSourceMode", {
      get: () => this.expandModeManager.expandSourceMode,
      set: (value) => { this.expandModeManager.expandSourceMode = value; },
    });

    // 레퍼런스 관련 프로퍼티 (ReferenceManager와 동기화를 위한 getter/setter)
    // 기존 코드 호환성을 위해 프로퍼티 유지 (ReferenceManager로 위임)
    Object.defineProperty(this, "selectedReferences", {
      get: () => this.referenceManager.getSelectedReferences(),
      set: (value) => this.referenceManager.setSelectedReferences(value),
    });
    Object.defineProperty(this, "referenceSelectionModal", {
      get: () => this.referenceManager.getReferenceSelectionModal(),
      set: (value) => this.referenceManager.setReferenceSelectionModal(value),
    });
    Object.defineProperty(this, "referenceLinkCache", {
      get: () => this.referenceManager.getReferenceLinkCache(),
      set: (value) => this.referenceManager.setReferenceLinkCache(value),
    });

    // ==================== SavedTextsManager 프로퍼티 위임 ====================
    // [Refactoring] 기존 코드 호환성을 위해 프로퍼티 유지 (SavedTextsManager로 위임)
    Object.defineProperty(this, "savedTexts", {
      get: () => this.savedTextsManager.savedTexts,
      set: (value) => {
        this.savedTextsManager.savedTexts = value;
      },
    });
    Object.defineProperty(this, "savedFilter", {
      get: () => this.savedTextsManager.savedFilter,
      set: (value) => {
        this.savedTextsManager.savedFilter = value;
      },
    });
    Object.defineProperty(this, "savedSearch", {
      get: () => this.savedTextsManager.savedSearch,
      set: (value) => {
        this.savedTextsManager.savedSearch = value;
      },
    });
    Object.defineProperty(this, "savedSearchDebounce", {
      get: () => this.savedTextsManager.savedSearchDebounce,
      set: (value) => {
        this.savedTextsManager.savedSearchDebounce = value;
      },
    });
    Object.defineProperty(this, "renderSavedTextsCache", {
      get: () => this.savedTextsManager.renderSavedTextsCache,
      set: (value) => {
        this.savedTextsManager.renderSavedTextsCache = value;
      },
    });
    Object.defineProperty(this, "renderSavedTextsCacheKey", {
      get: () => this.savedTextsManager.renderSavedTextsCacheKey,
      set: (value) => {
        this.savedTextsManager.renderSavedTextsCacheKey = value;
      },
    });
    Object.defineProperty(this, "renderSavedTextsDebounceTimer", {
      get: () => this.savedTextsManager.renderSavedTextsDebounceTimer,
      set: (value) => {
        this.savedTextsManager.renderSavedTextsDebounceTimer = value;
      },
    });
    Object.defineProperty(this, "availableTopics", {
      get: () => this.savedTextsManager.availableTopics,
      set: (value) => {
        this.savedTextsManager.availableTopics = value;
      },
    });
    Object.defineProperty(this, "availableSources", {
      ge1t: () => this.savedTextsManager.availableSources,
      set: (value) => {
        this.savedTextsManager.availableSources = value;
      },
    });

    // ==================== FilterManager 프로퍼티 위임 (Phase 9) ====================
    // [Refactoring] 기존 코드 호환성을 위해 프로퍼티 유지 (FilterManager로 위임)
    Object.defineProperty(this, "currentTopicFilter", {
      get: () => this.filterManager.getCurrentTopicFilter(),
      set: (value) => this.filterManager.setCurrentTopicFilter(value),
    });
    Object.defineProperty(this, "currentSourceFilter", {
      get: () => this.filterManager.getCurrentSourceFilter(),
      set: (value) => this.filterManager.setCurrentSourceFilter(value),
    });
    Object.defineProperty(this, "currentSnsFilterMode", {
      get: () => this.filterManager.getCurrentSnsFilterMode(),
      set: (value) => this.filterManager.setCurrentSnsFilterMode(value),
    });
    Object.defineProperty(this, "currentSnsFilterPlatform", {
      get: () => this.filterManager.getCurrentSnsFilterPlatform(),
      set: (value) => this.filterManager.setCurrentSnsFilterPlatform(value),
    });
    Object.defineProperty(this, "trackingSort", {
      get: () => this.filterManager.getTrackingSort(),
      set: (value) => this.filterManager.setTrackingSort(value),
    });
    Object.defineProperty(this, "trackingStatusFilter", {
      get: () => this.filterManager.getTrackingStatusFilter(),
      set: (value) => this.filterManager.setTrackingStatusFilter(value),
    });
    Object.defineProperty(this, "trackingSearch", {
      get: () => this.filterManager.getTrackingSearch(),
      set: (value) => this.filterManager.setTrackingSearch(value),
    });
    Object.defineProperty(this, "rangeFilters", {
      get: () => this.filterManager.getRangeFilters(),
      set: (value) => this.filterManager.setRangeFilters(value),
    });
    ...
    // currentTopicFilter는 FilterManager로 위임됨 (Object.defineProperty)
    // availableTopics는 SavedTextsManager로 위임됨 (Object.defineProperty)
    ...
    // currentSourceFilter는 FilterManager로 위임됨 (Object.defineProperty)
    // availableSources는 SavedTextsManager로 위임됨 (Object.defineProperty)
    ...
    // currentSnsFilterMode, currentSnsFilterPlatform은 FilterManager로 위임됨 (Object.defineProperty)
    ...
    // trackingSort, trackingStatusFilter, trackingSearch, rangeFilters는 FilterManager로 위임됨 (Object.defineProperty)
    ...
    this.currentUser = null;
    // savedTexts, savedFilter, savedSearch, savedSearchDebounce는
    // SavedTextsManager로 위임됨 (Object.defineProperty)
    ...
    // ========================================================================
    // SECTION 6: 추가 Manager 인스턴스 생성
    // ========================================================================

    // ==================== LLMValidationManager 인스턴스 생성 ====================
    // [Refactoring] LLM 검증 관련 상태를 LLMValidationManager로 위임
    this.llmValidationManager = new LLMValidationManager(this);

    // LLM 검증 관련 프로퍼티 (LLMValidationManager와 동기화를 위한 getter)
    // 기존 코드 호환성을 위해 프로퍼티 유지 (LLMValidationManager로 위임)
    Object.defineProperty(this, "llmPrompts", {
      get: () => this.llmValidationManager.getLLMPrompts(),
    });
    Object.defineProperty(this, "llmCharacteristics", {
      get: () => this.llmValidationManager.getLLMCharacteristics(),
    });
    Object.defineProperty(this, "llmUrls", {
      get: () => this.llmValidationManager.getLLMUrls(),
    });
  ...
  // ############################################################################
  // #                                                                          #
  // #  [Phase 10-02] 메서드 카테고리별 구조                                    #
  // #                                                                          #
  // #  1. 레퍼런스 관련 메서드 (ReferenceManager 위임)                         #
  // #  2. 초기화/이벤트 관련 메서드 (InitManager 위임)                         #
  // #  3. 텍스트 CRUD 관련 메서드 (TextCrudManager 위임)                       #
  // #  4. 저장된 글 관련 메서드 (SavedTextsManager 위임)                       #
  // #  5. 트래킹 관련 메서드 (TrackingManager 위임)                            #
  // #  6. 모달 관련 메서드 (ModalManager 위임)                                 #
  // #  7. 확대 모드 관련 메서드 (ExpandModeManager 위임)                       #
  // #  8. LLM 검증 관련 메서드 (LLMValidationManager 위임)                     #
  // #  9. 필터 관련 메서드 (FilterManager 위임)                                #
  // #  10. 유틸리티 메서드                                                     #
  // #                                                                          #
  // ############################################################################

  // ============================================================================
  // [CATEGORY 1] 레퍼런스 관련 메서드 - ReferenceManager로 위임
  // ============================================================================
  ...
  /**
   * 실시간 중복 체크 초기화
   * [Refactoring] ReferenceManager로 위임
   */
  initLiveDuplicateCheck() {
    this.referenceManager.initLiveDuplicateCheck();
  }
  ...
  showInlineDuplicateHint(duplicate) {
    this.referenceManager.showInlineDuplicateHint(duplicate);
  }
  ...
  hideInlineDuplicateHint() {
    this.referenceManager.hideInlineDuplicateHint();
  }
  ...
  initReferenceSelection() {
    // [Refactoring] ReferenceManager로 위임
    this.referenceManager.initReferenceSelection();
  }
   ...
   * [P3-04 이동됨] 실제 구현은 js/reference.js > ReferenceManager.initReferenceLoader()
   * @see ReferenceManager.initReferenceLoader
   */
  initReferenceLoader() {
    return this.referenceManager.initReferenceLoader();
  }
  ...
  // [P3-04 삭제됨] 구 버전 레퍼런스 로더 함수들이 ReferenceManager로 이동됨
  // openReferenceLoader, closeReferenceLoader, switchReferenceLoaderTab,
  // loadReferenceLoaderData, loadSavedReferencesForLoader, renderReferenceLoaderList,
  // handleReferenceItemClick은 js/reference.js로 이동되어 wrapper 메서드로 호출됨
  ...
  // ============================================================================
  // [Phase 10] 초기화 로직 - InitManager로 위임
  // ============================================================================

  /**
   * 앱 초기화 메인 진입점
   * [Phase 10] InitManager로 초기화 로직 위임
   */
  async init() {
    // InitManager를 통한 중앙 집중화된 초기화
    await this.initManager.initialize();
  }

  /**
   * [Deprecated] 이전 호환성을 위해 유지
   * 새 코드에서는 initManager.bindAllEvents() 사용 권장
   */
  bindEvents() {
    // [Phase 10] InitManager로 위임됨
    // 기존 코드 호환성을 위해 빈 메서드 유지
    // 실제 이벤트 바인딩은 initManager.bindAllEvents()에서 수행
  ...
  // ============================================================================
  // [Phase 10] 탭 관련 메서드 - TabManager로 위임
  // ============================================================================

  /**
   * 탭 기능 초기화
   * [Phase 10] TabManager로 위임됨 - 기존 코드 호환성을 위해 wrapper 유지
   */
  initTabListeners() {
    // TabManager가 초기화 시 자동으로 처리함
    // 기존 코드 호환성을 위해 메서드 유지
    if (this.initManager && this.initManager.tabManager) {
      // TabManager에서 이미 처리됨
      console.log('[DualTextWriter] initTabListeners() - TabManager로 위임됨');
    } else {
      // fallback: 기존 방식
   ...
   */
  switchTab(tabName) {
    // TabManager 사용 가능한 경우 위임
    if (this.initManager && this.initManager.tabManager) {
      this.initManager.tabManager.switchTab(tabName);
    } else {
      // fallback: 기존 방식
      this._legacySwitchTab(tabName);
    }
  }

  /**
   * [Deprecated] 기존 탭 전환 로직 - fallback용
   * @param {string} tabName - 탭 이름
   */
  _legacySwitchTab(tabName) {
  ...
  // ============================================================================
  // [Phase 10] DEPRECATED: 기존 bindEvents() 메서드
  // 이 코드는 InitManager로 이전되었습니다.
  // 호환성 테스트 완료 후 제거 예정
  // ============================================================================
  /**
   * @deprecated InitManager.bindAllEvents()로 대체됨
   * 이 메서드는 더 이상 호출되지 않습니다.
   */
  _legacyBindEvents_DEPRECATED() {
    // 사용자 인증 이벤트
  ...
  /**
   * 저장된 글 필터 UI 초기화 및 이벤트 바인딩 (SavedTextsManager로 위임)
   */
  initSavedFilters() {
    return this.savedTextsManager.initSavedFilters();
  }

  /**
   * 저장된 글 필터 설정 (SavedTextsManager로 위임)
   * @param {string} filter - 필터 값
   */
  setSavedFilter(filter) {
    return this.savedTextsManager.setSavedFilter(filter);
  }

  /**
   * 주제 필터 옵션 업데이트 (SavedTextsManager로 위임)
   */
  updateTopicFilterOptions() {
    return this.savedTextsManager.updateTopicFilterOptions();
  }

  /**
   * 소스 필터 옵션 업데이트 (SavedTextsManager로 위임)
   */
  updateSourceFilterOptions() {
    return this.savedTextsManager.updateSourceFilterOptions();
  }

  /**
   * SNS 필터 옵션 업데이트 (SavedTextsManager로 위임)
   */
  updateSnsFilterOptions() {
    return this.savedTextsManager.updateSnsFilterOptions();
  }
  ...
  normalizeContent(text) {
    return this.referenceManager.normalizeContent(text);
  }
  ...
  checkDuplicateReference(content) {
    return this.referenceManager.checkDuplicateReference(content);
  }

  /**
   * 내용 해시(SHA-256) 계산 (비동기)
   * [Refactoring] ReferenceManager로 위임
   * @param {string} content - 정규화된 내용
   * @returns {Promise<string>} 16진수 해시 문자열
   */
  async calculateContentHash(content) {
    return this.referenceManager.calculateContentHash(content);
  }
  ...
  calculateContentHashSync(content) {
    return this.referenceManager.calculateContentHashSync(content);
  }
   ...
   * 중복 레퍼런스 확인 모달 표시
   * [Refactoring] ReferenceManager로 위임
   * @param {Object} duplicate - 중복된 레퍼런스 객체
   * @returns {Promise<boolean>} 저장 진행 여부
   */
  ...
  // 아래 프로퍼티들은 SavedTextsManager로 위임됨 (Object.defineProperty 사용)
  // - renderSavedTextsDebounceTimer
  // - renderSavedTextsCache
  // - renderSavedTextsCacheKey

  /**
   * 저장된 글 목록 렌더링 (SavedTextsManager로 위임)
   * @returns {Promise} 렌더링 완료 Promise
   */
  async renderSavedTexts() {
    return this.savedTextsManager.renderSavedTexts();
  }

  /**
   * 휴지통 목록 렌더링 (SavedTextsManager로 위임)
   */
  renderTrashBinList() {
    return this.savedTextsManager.renderTrashBinList();
  }

  /**
   * 휴지통 열기 (SavedTextsManager로 위임)
   */
  openTrashBin() {
    return this.savedTextsManager.openTrashBin();
  }

  /**
   * 휴지통 닫기 (SavedTextsManager로 위임)
   */
  closeTrashBin() {
    return this.savedTextsManager.closeTrashBin();
  }

  /**
   * 저장된 글 목록 실제 렌더링 구현 (SavedTextsManager로 위임)
   * - 기존 코드 호환성을 위해 메서드 유지
   */
  async _renderSavedTextsImpl() {
    return this.savedTextsManager._renderSavedTextsImpl();
  }
...
현재는 로컬 스토리지 모드로 동작합니다.
  ...
  // ============================================================================
  // [P2-01] LLM 검증 시스템 초기화 - LLMValidationManager로 위임
  // ============================================================================
  // [Refactoring] 기존 initializeLLMValidation()은 constructor에서 LLMValidationManager 인스턴스 생성으로 대체됨
  // 하위 호환성을 위해 메서드는 유지하되, 실제 초기화는 LLMValidationManager에서 수행
  initializeLLMValidation() {
    // LLMValidationManager가 이미 초기화되어 있으면 로그만 출력
    if (this.llmValidationManager) {
      console.log("[P2-01] LLM 검증 시스템 이미 초기화됨 (LLMValidationManager 사용)");
      return;
    }
    console.warn("[P2-01] LLMValidationManager가 초기화되지 않았습니다.");
  }

  // ========================================
  // [P2-02] LLM 검증 실행 메서드 (LLMValidationManager로 위임)
  // ========================================

  /**
   * 패널 기반 LLM 검증 실행 (LLMValidationManager로 위임)
   * @param {string} panel - 패널 타입 ('reference' | 'writing')
   * @param {string} llmService - LLM 서비스 ID
   */
  async validatePanelWithLLM(panel, llmService) {
    if (this.llmValidationManager) {
      return this.llmValidationManager.validatePanelWithLLM(panel, llmService);
    }
    console.warn("[P2-02] LLMValidationManager가 초기화되지 않았습니다.");
  }

  /**
   * 저장된 글 기반 LLM 검증 실행 (LLMValidationManager로 위임)
   * @param {string} itemId - 저장된 글 ID
   * @param {string} llmService - LLM 서비스 ID
   */
  async validateWithLLM(itemId, llmService) {
    if (this.llmValidationManager) {
      return this.llmValidationManager.validateWithLLM(itemId, llmService);
    }
    console.warn("[P2-02] LLMValidationManager가 초기화되지 않았습니다.");
  }

  /**
   * 클립보드에 텍스트 복사 (LLMValidationManager로 위임)
   * @param {string} text - 복사할 텍스트
   */
  async copyToClipboard(text) {
    if (this.llmValidationManager) {
      return this.llmValidationManager.copyToClipboard(text);
    }
    // 폴백: LLMValidationManager가 없을 경우 직접 처리
    console.warn("[P2-02] LLMValidationManager가 초기화되지 않았습니다. 폴백 사용.");
    try {
  ...
  /**
   * LLM 사이트 새 탭에서 열기 (LLMValidationManager로 위임)
   * @param {string} llmService - LLM 서비스 ID
   * @param {string} text - 검증할 텍스트
   */
  openLLMSite(llmService, text) {
    if (this.llmValidationManager) {
      return this.llmValidationManager.openLLMSite(llmService, text);
    }
    console.warn("[P2-02] LLMValidationManager가 초기화되지 않았습니다.");
  }

  // ========================================
  // [P2-03] LLM 모달 관련 메서드 (LLMValidationManager로 위임)
  // ========================================

  /**
   * LLM 통합 복사 모달 표시 (LLMValidationManager로 위임)
   * @param {string} llmService - LLM 서비스 ID
   * @param {string} text - 복사할 텍스트
   */
  showLLMCopyModal(llmService, text) {
    if (this.llmValidationManager) {
      return this.llmValidationManager.showLLMCopyModal(llmService, text);
    }
    console.warn("[P2-03] LLMValidationManager가 초기화되지 않았습니다.");
  }

  /**
   * Gemini 전용 복사 모달 표시 (하위 호환성, LLMValidationManager로 위임)
   * @param {string} text - 복사할 텍스트
   */
  showGeminiCopyModal(text) {
    if (this.llmValidationManager) {
      return this.llmValidationManager.showGeminiCopyModal(text);
    }
    console.warn("[P2-03] LLMValidationManager가 초기화되지 않았습니다.");
  }

  /**
   * LLM 통합 텍스트 복사 함수 (LLMValidationManager로 위임)
   * @param {string} llmService - LLM 서비스 ID
   */
  copyLLMText(llmService) {
    if (this.llmValidationManager) {
      return this.llmValidationManager.copyLLMText(llmService);
    }
    console.warn("[P2-03] LLMValidationManager가 초기화되지 않았습니다.");
  }

  /**
   * Gemini 텍스트 복사 함수 (하위 호환성, LLMValidationManager로 위임)
   */
  copyGeminiText() {
    if (this.llmValidationManager) {
      return this.llmValidationManager.copyGeminiText();
    }
    console.warn("[P2-03] LLMValidationManager가 초기화되지 않았습니다.");
  }

  /**
   * LLM 검증 가이드 메시지 표시 (LLMValidationManager로 위임)
   * @param {string} llmService - LLM 서비스 ID
   */
  showLLMValidationGuide(llmService) {
    if (this.llmValidationManager) {
      return this.llmValidationManager.showLLMValidationGuide(llmService);
    }
    console.warn("[P2-03] LLMValidationManager가 초기화되지 않았습니다.");
  }

  /**
   * 상세 가이드 표시 (LLMValidationManager로 위임)
   * @param {string} llmService - LLM 서비스 ID
   */
  showDetailedGuide(llmService) {
    if (this.llmValidationManager) {
      return this.llmValidationManager.showDetailedGuide(llmService);
    }
    console.warn("[P2-03] LLMValidationManager가 초기화되지 않았습니다.");
  }
  ...
  // [Phase 10-02] 중복 escapeHtml 제거됨 - 3422줄의 메서드 사용
  // escapeHtml(text) → 이미 위에서 정의됨 (utils.js 위임)
  ...
  openReferenceSelectionModal() {
    this.referenceManager.openReferenceSelectionModal();
  }
  ...
  closeReferenceSelectionModal() {
    this.referenceManager.closeReferenceSelectionModal();
  }
  ...
  showLinkedReferencesModal(editId) {
    this.referenceManager.showLinkedReferencesModal(editId);
  }
  ...
  showEditsByReferenceModal(refId) {
    this.referenceManager.showEditsByReferenceModal(refId);
  }
  ...
  // ==================== 커스텀 모달 메서드 (Phase 6-03 ModalManager로 위임) ====================

  /**
   * 참고 레퍼런스 내용을 즉시 표시합니다.
   * [Refactoring] ModalManager로 위임
   * @param {string} referenceId - 레퍼런스 ID
   */
  showReferenceContentModal(referenceId) {
    this.modalManager.showReferenceContentModal(referenceId);
  }
  ...
  bindCustomModalEvents(modal) {
    this.modalManager.bindCustomModalEvents(modal);
  }
  ...
  renderReferenceSelectionList(references = null) {
    this.referenceManager.renderReferenceSelectionList(references);
  }
  ...
  bindReferenceCheckboxEvents() {
    this.referenceManager.bindReferenceCheckboxEvents();
  }
  ...
  updateReferenceSelectionCount() {
    this.referenceManager.updateReferenceSelectionCount();
  }
  ...
  toggleReferenceSelection(refId) {
    this.referenceManager.toggleReferenceSelection(refId);
  }
  ...
  confirmReferenceSelection() {
    this.referenceManager.confirmReferenceSelection();
  }
  ...
  renderSelectedReferenceTags() {
    this.referenceManager.renderSelectedReferenceTags();
  }
  ...
  bindReferenceTagRemoveEvents() {
    this.referenceManager.bindReferenceTagRemoveEvents();
  }
  ...
  removeSelectedReference(refId) {
    this.referenceManager.removeSelectedReference(refId);
  }
   ...
   */
  filterReferenceList() {
    this.referenceManager.filterReferenceList();
  }
  ...
  getLinkedReferences(editId) {
    return this.referenceManager.getLinkedReferences(editId);
  }
  ...
  getEditsByReference(referenceId) {
    return this.referenceManager.getEditsByReference(referenceId);
  }
  ...
  invalidateReferenceLinkCache() {
    this.referenceManager.invalidateCache();
  }
  ...
  initArticleManagement() {
    ...
    // 확대 모드 레퍼런스 상태
    // [Refactoring] expandReferences는 ReferenceManager로 위임
    Object.defineProperty(this, "expandReferences", {
      get: () => this.referenceManager.getExpandReferences(),
      set: (value) => this.referenceManager.setExpandReferences(value),
    });
    ...
    // [Refactoring] recentReferences는 ReferenceManager로 위임
    Object.defineProperty(this, "recentReferences", {
      get: () => this.referenceManager.getRecentReferences(),
      set: (value) => this.referenceManager.setRecentReferences(value),
    });
  ...
  // ===== 확대 모드 기능 (ExpandModeManager로 위임) =====
  // [Refactoring] Phase 7: 확대 모드 관련 기능을 ExpandModeManager로 위임

  /**
   * 확대 모드 열기 (Wrapper)
   * @see ExpandModeManager.openExpandMode
   */
  openExpandMode() {
    return this.expandModeManager.openExpandMode();
  }

  /**
   * 확대 모드 닫기 (Wrapper)
   * @see ExpandModeManager.closeExpandMode
   */
  closeExpandMode() {
    return this.expandModeManager.closeExpandMode();
  }

  /**
   * 저장하고 확대 모드 닫기 (Wrapper)
   * @see ExpandModeManager.saveAndCloseExpandMode
   */
  saveAndCloseExpandMode() {
    return this.expandModeManager.saveAndCloseExpandMode();
  }

  /**
   * 확대 모드 글자 수 카운터 업데이트 (Wrapper)
   * @see ExpandModeManager.updateExpandContentCounter
   */
  updateExpandContentCounter() {
    return this.expandModeManager.updateExpandContentCounter();
  }

  /**
   * 확대 모드에 레퍼런스 추가 (Wrapper)
   * @see ReferenceManager.addReferenceToExpandMode
   */
  addReferenceToExpandMode(item, sourceType) {
    return this.referenceManager.addReferenceToExpandMode(item, sourceType);
  }

  /**
   * 확대 모드에서 레퍼런스 제거 (Wrapper)
   * @see ExpandModeManager.removeExpandReference
   */
  removeExpandReference(index) {
    return this.expandModeManager.removeExpandReference(index);
  }

  /**
   * 확대 모드 레퍼런스 렌더링 (Wrapper)
   * @see ExpandModeManager.renderExpandReferences
   */
  renderExpandReferences(newlyAddedId = null) {
    return this.expandModeManager.renderExpandReferences(newlyAddedId);
  }

  /**
   * 확대 모드 레퍼런스를 내용 필드에 추가 (Wrapper)
   * @see ExpandModeManager.addExpandReferenceToContent
   */
  addExpandReferenceToContent(ref, index) {
    return this.expandModeManager.addExpandReferenceToContent(ref, index);
  }

  /**
   * 확대 모드 레퍼런스 패널 토글 (Wrapper)
   * @see ExpandModeManager.toggleExpandReferencePanel
   */
  toggleExpandReferencePanel() {
    return this.expandModeManager.toggleExpandReferencePanel();
  }

  /**
   * 확대 모드 분할선 드래그 초기화 (Wrapper)
   * @see ExpandModeManager.initExpandSplitResize
   */
  initExpandSplitResize() {
    return this.expandModeManager.initExpandSplitResize();
  }

  // ==================== 레퍼런스 로더 Wrapper 메서드 (P3-04) ====================
  // [이동됨] 실제 구현은 js/reference.js > ReferenceManager 클래스로 이동

  /**
   * 레퍼런스 로더 열기 (Wrapper)
   * @see ReferenceManager.openReferenceLoader
   */
  openReferenceLoader() {
    return this.referenceManager.openReferenceLoader();
  }

  /**
   * 레퍼런스 로더 닫기 (Wrapper)
   * @see ReferenceManager.closeReferenceLoader
   */
  closeReferenceLoader() {
    return this.referenceManager.closeReferenceLoader();
  }

  /**
   * 레퍼런스 탭 전환 (Wrapper)
   * @see ReferenceManager.switchReferenceTab
   */
  switchReferenceTab(tabName) {
    return this.referenceManager.switchReferenceTab(tabName);
  }

  /**
   * 레퍼런스 검색 처리 (Wrapper)
   * @see ReferenceManager.handleReferenceSearch
   */
  handleReferenceSearch(query) {
    return this.referenceManager.handleReferenceSearch(query);
  }

  /**
   * 레퍼런스 목록 로드 (Wrapper)
   * @see ReferenceManager.loadReferenceList
   */
  async loadReferenceList() {
    return this.referenceManager.loadReferenceList();
  }

  /**
   * 저장된 글 레퍼런스 로드 (Wrapper)
   * @see ReferenceManager.loadSavedReferences
   */
  async loadSavedReferences(searchQuery = "", categoryFilter = "") {
    return this.referenceManager.loadSavedReferences(searchQuery, categoryFilter);
  }

  /**
   * 트래킹 레퍼런스 로드 (Wrapper)
   * @see ReferenceManager.loadTrackingReferences
   */
  async loadTrackingReferences(searchQuery = "", categoryFilter = "", sortFilter = "recent") {
    return this.referenceManager.loadTrackingReferences(searchQuery, categoryFilter, sortFilter);
  }
  ...
  /**
   * 레퍼런스 목록 렌더링 (Wrapper)
   * @see ReferenceManager.renderReferenceList
   */
  renderReferenceList(items, container, sourceType) {
    return this.referenceManager.renderReferenceList(items, container, sourceType);
  }

  /**
   * 레퍼런스 아이템 생성 (Wrapper)
   * @see ReferenceManager.createReferenceItem
   */
  createReferenceItem(item, sourceType) {
    return this.referenceManager.createReferenceItem(item, sourceType);
  }

  /**
   * 레퍼런스를 확대 모드의 레퍼런스 영역에 추가 (Wrapper)
   * @see ReferenceManager.addReferenceToContent
   */
  addReferenceToContent(item, sourceType) {
    return this.referenceManager.addReferenceToContent(item, sourceType);
  }

  /**
   * 레퍼런스를 확대 모드에 추가하고 사용자에게 알림 (Wrapper)
   * @see ReferenceManager._addReferenceToExpandModeAndNotify
   * @private
   */
  _addReferenceToExpandModeAndNotify(item, sourceType, isNewlyOpened) {
    return this.referenceManager._addReferenceToExpandModeAndNotify(item, sourceType, isNewlyOpened);
  }

  /**
   * 최근 사용 레퍼런스 로드 (localStorage) - 유지
   * 여러 곳에서 사용되므로 wrapper가 아닌 원본 유지
   */
  ...
  /**
   * 최근 사용 레퍼런스 목록 렌더링 (Wrapper)
   * @see ReferenceManager.loadRecentReferencesList
   */
  async loadRecentReferencesList() {
    return this.referenceManager.loadRecentReferencesList();
  }
...
// ============================================================================
// [P1-04] 메트릭 관리 메서드 - TrackingManager로 위임
// ============================================================================
// 메트릭 관리 모달 열기 (트래킹 탭에서 사용)
// [Refactoring] TrackingManager로 위임
DualTextWriter.prototype.manageMetrics = async function (postId) {
  if (this.trackingManager && this.trackingManager.manageMetrics) {
    return this.trackingManager.manageMetrics(postId);
  }
...
DualTextWriter.prototype.renderMetricsListForManage = function (
  ...
  if (this.trackingManager && this.trackingManager.renderMetricsListForManage) {
    return this.trackingManager.renderMetricsListForManage(metrics, postId, textId);
  }
  console.warn('[DualTextWriter] trackingManager.renderMetricsListForManage를 찾을 수 없습니다.');
  return '<div style="text-align: center; padding: 40px; color: #666;">메트릭 데이터를 불러올 수 없습니다.</div>';
};

// bindMetricsManageEvents - TrackingManager로 위임
DualTextWriter.prototype.bindMetricsManageEvents = function (postId, textId) {
  if (this.trackingManager && this.trackingManager.bindMetricsManageEvents) {
    return this.trackingManager.bindMetricsManageEvents(postId, textId);
  }
  console.warn('[DualTextWriter] trackingManager.bindMetricsManageEvents를 찾을 수 없습니다.');
};

// editMetricFromManage - TrackingManager로 위임
DualTextWriter.prototype.editMetricFromManage = async function (
  postId,
  textId,
  metricIndex
) {
  if (this.trackingManager && this.trackingManager.editMetricFromManage) {
    return this.trackingManager.editMetricFromManage(postId, textId, metricIndex);
  }
  console.warn('[DualTextWriter] trackingManager.editMetricFromManage를 찾을 수 없습니다.');
};

// deleteMetricFromManage - TrackingManager로 위임
DualTextWriter.prototype.deleteMetricFromManage = async function (
  postId,
  textId,
  metricIndex
) {
  if (this.trackingManager && this.trackingManager.deleteMetricFromManage) {
    return this.trackingManager.deleteMetricFromManage(postId, textId, metricIndex);
  }
  console.warn('[DualTextWriter] trackingManager.deleteMetricFromManage를 찾을 수 없습니다.');
};

// bindBatchSelectEvents - TrackingManager로 위임
DualTextWriter.prototype.bindBatchSelectEvents = function (postId, textId) {
  if (this.trackingManager && this.trackingManager.bindBatchSelectEvents) {
    return this.trackingManager.bindBatchSelectEvents(postId, textId);
  }
  console.warn('[DualTextWriter] trackingManager.bindBatchSelectEvents를 찾을 수 없습니다.');
};

// updateBatchSelectUI - TrackingManager로 위임
DualTextWriter.prototype.updateBatchSelectUI = function () {
  if (this.trackingManager && this.trackingManager.updateBatchSelectUI) {
    return this.trackingManager.updateBatchSelectUI();
  }
  console.warn('[DualTextWriter] trackingManager.updateBatchSelectUI를 찾을 수 없습니다.');
};

// refreshMetricsListForManage - TrackingManager로 위임
DualTextWriter.prototype.refreshMetricsListForManage = async function (
  postId,
  textId
) {
  if (this.trackingManager && this.trackingManager.refreshMetricsListForManage) {
    return this.trackingManager.refreshMetricsListForManage(postId, textId);
  }
  console.warn('[DualTextWriter] trackingManager.refreshMetricsListForManage를 찾을 수 없습니다.');
};

// batchDeleteMetrics - TrackingManager로 위임
DualTextWriter.prototype.batchDeleteMetrics = async function (postId, textId) {
  if (this.trackingManager && this.trackingManager.batchDeleteMetrics) {
    return this.trackingManager.batchDeleteMetrics(postId, textId);
  }
  console.warn('[DualTextWriter] trackingManager.batchDeleteMetrics를 찾을 수 없습니다.');
};
...
// [P1-02] 분석 데이터 계산 - TrackingManager로 위임
DualTextWriter.prototype.calculateAnalytics = function (metrics) {
  // TrackingManager로 위임 (Firebase 의존성 주입 패턴)
  if (this.trackingManager) {
    return this.trackingManager.calculateAnalytics(metrics);
  }
  console.warn('[calculateAnalytics] TrackingManager가 초기화되지 않았습니다.');
  return {};
};

// [P1-02] 트래킹 요약 업데이트 - TrackingManager로 위임
DualTextWriter.prototype.updateTrackingSummary = function () {
  // TrackingManager로 위임 (Firebase 의존성 주입 패턴)
  if (this.trackingManager) {
    return this.trackingManager.updateTrackingSummary();
  }
  console.warn('[updateTrackingSummary] TrackingManager가 초기화되지 않았습니다.');
};

// [P1-03] 트래킹 차트 초기화 - TrackingManager로 위임
DualTextWriter.prototype.initTrackingChart = function () {
  // TrackingManager로 위임 (Chart.js 의존성 주입 패턴)
  if (this.trackingManager) {
    return this.trackingManager.initTrackingChart();
  }
  console.warn('[initTrackingChart] TrackingManager가 초기화되지 않았습니다.');
};

// [P1-03] 트래킹 차트 업데이트 - TrackingManager로 위임
DualTextWriter.prototype.updateTrackingChart = function () {
  // TrackingManager로 위임
  if (this.trackingManager) {
    return this.trackingManager.updateTrackingChart();
  }
  console.warn('[updateTrackingChart] TrackingManager가 초기화되지 않았습니다.');
};
...
DualTextWriter.prototype.setScaleMode = function (mode) {
  // [P1-03] TrackingManager로 위임
  if (this.trackingManager) {
    return this.trackingManager.setScaleMode(mode);
  }
  console.warn('[setScaleMode] TrackingManager가 초기화되지 않았습니다.');
};
...
DualTextWriter.prototype.setChartMode = function (mode) {
  // [P1-03] TrackingManager로 위임
  if (this.trackingManager) {
    return this.trackingManager.setChartMode(mode);
  }
  console.warn('[setChartMode] TrackingManager가 초기화되지 않았습니다.');
};
...
DualTextWriter.prototype.setChartRange = function (range) {
  // [P1-03] TrackingManager로 위임
  if (this.trackingManager) {
    return this.trackingManager.setChartRange(range);
  }
...
// [P1-03] 중복된 updateTrackingChart 메서드 제거됨 - TrackingManager로 위임
// 기존 코드는 js/tracking.js의 TrackingManager.updateTrackingChart()로 이동됨
...
DualTextWriter.prototype.checkReferenceUsage = async function (referenceTextId) {
  if (this.referenceManager && this.referenceManager.checkReferenceUsage) {
    return this.referenceManager.checkReferenceUsage(referenceTextId);
  }
  console.warn('[checkReferenceUsage] ReferenceManager? ????? ?????.');
  return 0;
};
...
DualTextWriter.prototype.checkMultipleReferenceUsage = async function (
  ...
  return this.referenceManager.checkMultipleReferenceUsage(referenceTextIds);
};
// ============================================================================
// [P3-02] ???? ??/?? - ReferenceManager? ??
// ============================================================================
DualTextWriter.prototype.markReferenceAsUsed = async function (referenceTextId) {
  if (this.referenceManager && this.referenceManager.markReferenceAsUsed) {
    return this.referenceManager.markReferenceAsUsed(referenceTextId);
  }
  console.warn('[markReferenceAsUsed] ReferenceManager? ????? ?????.');
};

DualTextWriter.prototype.unmarkReferenceAsUsed = async function (referenceTextId) {
  if (this.referenceManager && this.referenceManager.unmarkReferenceAsUsed) {
    return this.referenceManager.unmarkReferenceAsUsed(referenceTextId);
  }
  console.warn('[unmarkReferenceAsUsed] ReferenceManager? ????? ?????.');
};
...
// ============================================================================
// [P3-03] Orphan ??? ?? - ReferenceManager? ??
// ============================================================================
DualTextWriter.prototype.cleanupOrphanPosts = async function () {
  if (this.referenceManager && this.referenceManager.cleanupOrphanPosts) {
    return this.referenceManager.cleanupOrphanPosts();
  }
  console.warn('[cleanupOrphanPosts] ReferenceManager? ????? ?????.');
};
...
// ============================================================================
// [Phase 10-03] 전역 함수 정리
// - 인스턴스 생성: DOMContentLoaded에서 처리
// - 인라인 핸들러 호환성 함수: 프로덕션에서도 필요 (HTML onclick 등에서 호출)
// ============================================================================
...
// ============================================================================
// 인라인 핸들러 호환성 함수 (프로덕션 필수)
// HTML의 onclick 등에서 직접 호출되므로 전역 노출 필요
// ============================================================================

/**
 * 트래킹 데이터 저장 (인라인 핸들러 호환)
 */
...
// ============================================================================
// [Phase 10-03] 아티클 관련 전역 함수 (프로덕션 필수)
// 기존 코드에서 직접 호출되므로 전역 노출 필요
// ============================================================================
